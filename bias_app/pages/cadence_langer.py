"""Cadence-aware Langer model page."""
...
from components.param_panels import build_cadence_langer_params
...
dash.register_page(
    ...
    path='/cadence-langer',
    name='Cadence (L)',
    title='Bias Correction — Cadence Langer',
    order=3,
    icon='tabler:clock-star',
)

PREFIX = 'cadence-langer'
...
layout = dmc.Container([
    ...
    dmc.Grid([
        dmc.GridCol(build_cadence_langer_params(PREFIX), span=4),
        dmc.GridCol(dmc.Stack([
            dmc.Title('Cadence-Aware Langer', order=3),
            dmc.Text('Deterministic cadence matching + Langer 2020 periods', size='sm', c='dimmed'),
...
register_result_browser_callbacks(PREFIX, model_filter='cadence_langer')
register_sim_plot_callbacks(PREFIX)

from callbacks.simulation_cadence_cb import register_cadence_simulation_callback
register_cadence_simulation_callback(PREFIX, period_model='langer2020')
