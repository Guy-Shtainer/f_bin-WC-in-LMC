...
### 2026-04-28 — Validation-tab Sprint 4 Phases 5–8 + 9-panel CDF cross-method standardisation (uncommitted)

**What was done:**
- **Phase 5 — five regression bugs from the 2026-04-23 Sprint 4.** (1c) Best-fit CDF flat at 0.12 — root cause was `app/bc/render_shared.py:191` blindly squeezing the leading axis (`p_arr[0]`) instead of size-1 axes, which corrupted `best_vals['logPmax']` to NaN and routed the renderer through a broken back-compat sim path with a default `BinaryParameterConfig`. Fix: mirror the cadence_langer "find first size-1 axis, then `np.squeeze(axis=ax)`" pattern; if no size-1 axis exists, fall back to `arr[0]` only as a last resort. (1e) Sanity Check `Unknown period_model: dsilva` — caller passed the runner-mode tag `'dsilva'` as `period_model`, but `sample_logP` accepts only `'powerlaw'` / `'langer2020'`. Translation fix in three callers (`render_lk_explorer.py:1357`, `render_lk.py:549`, `render_lk_explorer_langer.py:1062`); E051 added to `COMMON_ERRORS.md` with prevention guidance ("translate at the data-source boundary"). (1b argmax) `argmax_logPmax = NaN` — same root cause as 1c; the squeeze fix cured it. (1b HDI) Per user instruction "show the maximum point in the marginalized graphs", added a new "Marginal max" column alongside the canonical "Joint argmax"; updated `feedback_honest_labels.md` to allow the marginal max if explicitly labelled. (1a) A&A theme: a single `shared.make_heatmap_fig` root edit fixed ~10 heatmaps; user authorised WORKING-flag edits inside Model Explorer (D17), corner plots (D14), parabola fits (D9).
- **Phase 6 — bootstrap-band purge + CDF style constants.** Flagged the diagnostic table as `# WORKING` ("perfect, mark it as working"). Removed false bootstrap bands from the mock-preview CDF + binary-fraction-vs-threshold panel (user: "feels like madeup useless confusing unrelated false data"). Added single-source-of-truth CDF style constants `_CDF_OBS_COLOR`, `_CDF_FIT_COLOR`, `_CDF_OBS_MARKER`. Standardised five CDFs (mock-preview, best-fit comparison, sanity check, re-sim interp, model explorer) on teal step + dashed red sim.
- **Phase 7 — truth-coded per-star dots on CDFs 2–5.** After user pushback "they are certainly not visually consistent", added `validation_io.load_per_star_truth(result)` helper (loads `is_binary` from `mock_stars.npz`); rendered truth-coded red/green per-star dot overlays on CDFs 2–5 (Dsilva) + the three Langer twins matching the first CDF's pattern; replaced 14+ remaining hardcoded colours with the new constants.
- **Phase 8 — Panel B alignment + bootstrap-band methodology preserved.** Aligned `_render_panel_b_cdf` colours to the teal/red constants; added `shape='hv'` to the sim trace + band fill + residual; explained that Panel B's bootstrap band IS computed differently (sampling uncertainty at N = 25 vs grid sim variance) — kept the methodology, only changed appearance.
- **Round 5/6 — 9-panel CDF cross-method standardisation.** User reported four bugs in Bias Correction → Validation CDF panels (dot positioning off, inconsistent colours, missing marginal best-fit, draw clutter). Standardised the convention across nine plot sites in six files: mock observation = black solid step (sorted-raw, NOT binned), grid-argmax best-fit = red `#D62728` dashed median + 16/84 shadow + faint draws, marginal-posterior best-fit (NEW overlay) = purple `#9467BD` dashed median + 16/84 shadow + faint draws. Marginal best-fit reuses the canonical `_method_best_and_hdi` → `info['hdi'][name][0]` (no re-derivation). In the multi-method "CDF Comparison" panel the red/purple convention is GLOBAL — methods are distinguished by legend label + a band-opacity ramp 0.18 → 0.12 → 0.08. Dot-positioning bug root cause: the obs step was drawn from `_binned_cdf` (coarse ~5 km/s edges) while the dots used raw star ΔRV; the fix builds both from the same sorted array so the dots sit exactly on step corners.

**Key results:**
- Five regression bugs from end-of-day 2026-04-23 closed at the structural level: best-fit CDF tracks the heatmap-best curve, sanity-check no longer raises on Dsilva, `argmax_logPmax` is finite, joint-argmax + marginal-max columns coexist with explicit honest labels, all heatmaps render on white-bg A&A theme.
- Nine CDF panels across six files now share a single colour convention; per-star truth dots overlay every CDF that has a corresponding mock truth.
- New `feedback_squeeze_axis.md` rule added to `MEMORY.md`: "never blindly drop a leading axis (`arr[0]`); squeeze ONLY size-1 axes, then verify each remaining axis against its named grid."

**Methodology notes for paper:**
- The "joint argmax + marginal max" honest-labels convention is a deliberate two-statistic display: the joint argmax is the canonical posterior summary, and the marginal max is shown alongside (with explicit "Marginal max" column header) so a reader can see at a glance whether the two agree. When they disagree by more than the HDI68 width, the parameter is strongly correlated with at least one nuisance axis.
- Panel B's bootstrap band (sampling uncertainty at N = 25) is methodologically distinct from CDFs 2–5's stored grid variance (Monte-Carlo noise of the grid simulation). Both are statistically valid and answer different questions; we kept the bootstrap on Panel B and the grid variance on the other four.

**Decisions:**
- Sprint 4 still uncommitted on `main` — visual sign-off pending.
- Did NOT touch the three deferred white-on-white traces inside WORKING-flagged blocks (`render_lk_explorer.py:973`, `:995`, `_langer.py:284`) per earlier instruction.
- The honest-labels rule was relaxed at the user's explicit request (per `feedback_follow_commands.md`) to allow the marginal max if labelled honestly. Memory updated rather than ignored.

**Bugs found and fixed:**
- E051 — Runner-mode tag passed as `period_model` value: caller passes `'dsilva'`/`'langer'` (runner-mode tag) but `sample_logP` accepts only `'powerlaw'`/`'langer2020'`. Fix at the boundary, not at every consumer. Added to `COMMON_ERRORS.md` with grep pattern `period_model\s*=\s*['\"]dsilva['\"]\|period_model\s*=\s*['\"]langer['\"]`.
- Squeeze-axis pattern (`p_arr[0]` instead of `np.squeeze(axis=ax)`) — already covered by E039; codified the rule in a dedicated memory file (`feedback_squeeze_axis.md`) so future agents flag it before committing.
- Dot-positioning bug on validation CDFs — design issue (binned-step + raw-dots mismatch); not greppable, no `COMMON_ERRORS.md` entry.

**Open questions:**
- Visual sign-off on all nine CDF panels in both Dsilva and Langer modes still pending.
- "Missing 1D normalized + 1D unnormalized + 3D interpolation graphs near heatmaps" — could not reproduce offline; coder added defensive `st.warning` to surface the next failure. User may need to clear cache and reload.

---

### 2026-04-29 — Validation-tab mock-data overhaul + paper writing session (figures + table + LaTeX bugs + Shenar polish)

**What was done:**
- **Validation-tab mock-data overhaul.** Extended `_sample_delta_rv_mock` and `generate_mock_observations_detail` with optional `sigma_meas_binary`, `error_model_binary`, `error_params_binary` (defaults `None` fall through to the single side's values; the Explorer's CDF sanity-check therefore stays byte-identical). Replaced the single error-model selector with a two-column Singles \| Binaries layout mirroring the Dsilva/Langer cadence-tab pattern. Added horizontal ΔRV error bars on every dot in the CDF + binary-fraction panels using `σ_ΔRV = √(σ_min² + σ_max²)` (per-epoch errors at min/max RV epochs — the same formula as `02_spectrum.py`). Detection curves now apply BOTH criteria: `(ΔRV > t)` AND `(ΔRV − 4σ > 0)`. Star table gained `σ_ΔRV (km s⁻¹)` and `Detected (2-crit)` columns. Critical bug fix: mock binary count was Bernoulli-drawn (`rng.random(N) < f_bin`), so seed = 42 + f_bin = 0.7 + N = 25 produced 13 binaries (2σ-low) instead of the expected 17–18; switched to deterministic `n_bin = int(round(N × f_bin))` with `rng.choice` selecting which stars are binary; verified across 8 fbin values × 5 seeds. True Parameter sliders (f_bin, π, σ_single, logP_max) and the random seed now persist to `settings/user_settings.json` via `SettingsManager.save([section, val_*])` on every change; defaults read from `grid_cadence_dsilva` / `grid_cadence_langer` config on load (independent per period model). Reverted "Realised f_bin" from the entire page after user pushback ("drop it entirely... I talked about the entire page") — caption mention, dotted horizontal line on binary-fraction panel, Panel A red dashed line, legend `(N)` count suffix all removed.
- **Paper writing session — figures inserted/rendered + table populated + LaTeX bugs + Shenar-style polish.** Inserted 13 figure environments across five section files; 11 PDFs rendered to `plots/` from real data via the new 1664-line `pipeline/export_paper_figs.py` driver (cdf_obs_vs_sim, fbin_pi_heatmap, fbin_pi_marginals, langer_heatmap, peak_drv_per_star, agreement, bin_sensitivity, period_models, sample_map, ccf_profile, binary_examples — all A&A-styled). Populated `paper/tables/star_sample.tex` with 25 stars × (Name, BAT99, RA, Dec, Group, Subtype, N_epochs, Source) + four footnotes (Brey 70a WN/WC transition, HD 269891 WN+B, RMC 140 WN+WC composite, Brey 83 WC5 → WC4 reclassification). Resolved the H2013 LMCe 584 ≡ L72 LH 41-1042 substitution via `specs.py` + FITS-header verification (1.5″ pointing offset). Per-row epoch counts now reflect actual data (5/6/7 split: 3 + 15 + 7 = 25). BAT99 + spectral types looked up from the Breysacher+1999 PDF (VizieR was returning PostgreSQL errors); Foellmi+2003 and Schnurr+2008 added to `references.bib`. Fixed eight distinct LaTeX bugs causing the "small + glued + no spaces" rendering pathology: math-mode contamination in unit macros (`\kms`, `\Msun`, `\Lsun`, `\Zsun`, `\Mdot` embedded `$...$` toggles instead of `\ensuremath{}`), `\fbincorr^{...}_{...}` atom-binding malformations (missing `{}` wrap on the macro before super/subscripts), `\tablefoot{}` blocks without `\par` separators, and the silent killer — `tables/star_sample.tex` was never `\input`-ed, so the populated table was invisible in the PDF and `\ref{tab:sample}` rendered as `??`. Applied seven Shenar-style fixes: deleted dead `abstract.tex`, populated the empty Conclusions block, narrative Conclusions paragraphs (no `enumerate`), `\tablefoot{}` on `tab:emission_lines`, populated the appendix placeholder, ESO programmes via the `\esoProgrammes` macro. Methods grew from ~150 to ~310 lines: added §Choice of primary RV line, §Line-agreement cross-check, §ΔRV threshold derivation, refined §Threshold-curve fit, §Spectroscopic morphology investigation. Discussion + Conclusions fully drafted (forward-looking, macro-driven, awaiting Phase-4 number lockdown). Five commits to local `main` + seven pushes to Overleaf (latest Overleaf master `0a5a359`).

**Key results:**
- Validation tab: mock binary count is now exact (`n_bin = round(25 × f_bin)`); detection curves use the same 2-criteria classifier as the real-observation pipeline; per-epoch error bars give the visual cue that a "borderline" detection comes with quantified uncertainty.
- Paper: 9 sections + abstract + tables + 11 figure PDFs are live on Overleaf; the star sample table is fully populated; all major prose sections drafted; submission-track shape with five user-decisions remaining (sample-purity decisions on HD 269891 + Brey 70a, the user-owned `fig:threshold_derivation`, ESO programme IDs, funding-source acknowledgement, Dsilva HDI pathology re-run).

**Methodology notes for paper:**
- **Validation-tab convention — deterministic mock binary count.** The validation page treats the input `f_bin` as a *target* fraction (rounded to integer), not as a Bernoulli probability. Rationale: the user must be able to trust that "70 % input" produces 70 % (rounded) so the recovery test is a like-for-like comparison. Bernoulli interpretation is mathematically defensible but unhelpful for sanity-checking the recovery pipeline at small N = 25.
- **Validation-tab convention — UI persistence.** All slider/number-input/selector values on the validation page persist to `user_settings.json` on every change; defaults read from the active config on load. Codified in `feedback_ui_persist_on_change.md` so future UI work follows the same pattern.
- **Paper — math-mode contamination cluster.** Unit macros embedding `$...$` toggles (instead of `\ensuremath{...}`), super/subscripts on `\xspace`-terminated macros (instead of `{...}` atom binding), and `\tablefoot{}` blocks without `\par` separators are all observable as "small + glued + no spaces" rendering. Prevention: agents should grep for `$\\\\` `}\$`, `\^{[+-]\\` (super/subscript on a backslash-prefixed macro), and `\\tablefoottext{[^}]*}\\tablefoottext{` (footnote-run-together) before claiming a clean LaTeX edit.
- **Paper — two-stage figure rendering coordination.** Writer inserts the figure environment with a commented `\includegraphics` + caption; plots agent renders the PDF with a matched filename; orchestrator uncomments via `sed` after both finish. File naming must be locked upfront so the find-and-replace step is unambiguous.

**Decisions:**
- Validation page shows NO realised-fraction information anywhere (caption, dotted line, legend suffix, Panel A red dashed line all reverted) — rationale: realised count ≈ round(input × N) is trivial, confusing, and adds no information.
- Backward-compat default for the new binary error args: `None` → fall through to single's values. Preserves the byte-identical contract with the Explorer (single error model gives an identical mock as before for those callers).
- Writer agent owns the full `paper/**/*.tex` edit cycle end-to-end — no coder middleware. Codified in `feedback_writer_owns_tex_edits.md` after a mid-session correction.
- No `% TODO` comments in paper LaTeX — track gaps via TBD-resolving macros only. The paper source is a deliverable; journal notes are not. Codified in `feedback_no_todos_in_paper.md`.
- Style: K-S test fully purged from the paper (multinomial likelihood is the only scoring method).
- Style: dropped two-Gaussian f_bin extraction from `fig:threshold_derivation` (the model can't constrain f_bin from the cumulative curve alone — degeneracy persists even with σ_s fixed). User produces this figure themselves with a folded-normal `2·Φ(T/σ)` model.

**Bugs found and fixed:**
- Mock binary count off by 2σ at small N (Bernoulli vs deterministic): not greppable but the convention is now codified in `feedback_mock_deterministic.md`.
- Eight LaTeX bugs causing "small + glued + no spaces" rendering — three macro-embedding-`$...$` bugs, two atom-binding bugs, one `\par`-missing footnote bug, one silent missing `\input` bug. Each individually trivial; the pattern is the danger.

**Open questions:**
- 17 TBD macros pending Phase-4 number lockdown (`\fbincorr`, `\fbinerrup`, …); when the bias-correction tuning converges, also flip Conclusions verb tense from forward-looking to past tense and render `fig:lmc_vs_mw`.
- Sample-purity decision on HD 269891 (WN+B per Schnurr+2008) and Brey 70a (WN/WC per Foellmi+2003): if excluded, `\Nstars` → 23 or 24 and the Bartzakos+2001 28-star anchor needs revisiting.
- `binary_classification.tex` rows still empty (header only) — populate after final classifications.
- ADS bibcode verification on the ~16 references via the `paper-research` skill (read-only ADS lookup) — pending Phase 5 final compile.

---

### 2026-04-30 — Three-stage binarity strategy memo + Likelihood Model Explorer ↔ grid score parity

**What was done:**
- **Three-stage binarity strategy + 5 validation runs queued.** Captured Guy's proposed three-stage workflow as a project memory: Stage 1 = bias correction with no f_bin / threshold assumed, finds best (π, logP_max[, σ_single]); Stage 2 = use the winning model to draw many systems (cadence-free) and fit a parametric distribution to the binary RVs; Stage 3 = plug that distribution into the RV Modeling tool to solve for the observed f_bin, threshold, and σ_single. Logged the open error-propagation question: passing a single best-fit point loses the corner-plot uncertainty; two candidate framings (sample marginals → distribution-of-distributions, or track how Stage-2 PDF parameters scatter across Stage-1 posterior draws). Captured five validation simulations launched the same evening for next-day review: Tabs 1–2 (low/high f_bin extremes with σ_single pinned to truth), Tabs 3–4 (sensitivity to an extra histogram bin near the steep CDF slope), Tab 5 on port 8502 (σ_single included as a free search dimension). Linked the σ_single-search run on Tab 5 to the three-stage memo as the decisive test for whether σ_single can live in Stage 1 or must be deferred to Stage 3.
- **Likelihood Model Explorer ↔ grid score parity.** Traced both score paths side-by-side: the Explorer's `_me_cdf_band` (validation_mode → `_sample_delta_rv_mock` with mock seed and forced `n_sets = 1`; real-obs → `simulate_delta_rv_cadence_aware` with hardcoded `default_rng(42)`) vs the grid worker's `_single_grid_task_cadence_aware` (`default_rng(seed_base + idx_cell)`, full `n_sets`, multinomial against `likelihood_bin_edges`). Confirmed CDF visualisation bins (`bin_edges`, default 36-edge `np.arange(0, 360, 10)` or adaptive) are completely independent of likelihood scoring bins (`likelihood_bin_edges`, default Dsilva 4-bin `[0, 45.5, 250, 650, ∞]`); the 36 CDF edges only feed the median + 16/84 CDF arrays for plotting, while the score uses the raw pooled ΔRV and `lk_be` only. Three coder-agent batches landed: (1) deterministic `n_bin = round(N × f_bin)` in both `simulate_delta_rv_sample:715` and `simulate_delta_rv_cadence_aware:930`; new `_explorer_run_grid_pipeline` helper; gated Explorer score + CDF behind a 🔬 Run button; dropped `validation_mode` from `_me_cdf_band`; applied to both Dsilva and Langer twins. (2) After user-reversed direction (auto-update is fast enough), removed the Run button; added a cached wrapper `_explorer_run_grid_pipeline_cached` keyed on `(f_bin, π, σ, logP_max, n_sets, seed_base + idx_cell, hashable cfg)`; promoted `_n_sets_me` to a user-editable `st.number_input` (default = `result.get('grid_n_sets', 1000)`, no max, step = 100, persisted to `lk_explorer.n_sets` / `lk_explorer_langer.n_sets`); auto-recompute on every slider/number-input change with a "Computing logL…" spinner only on cache miss. The global best metric still reads directly from `logL_raw[i_sig_best, i_fb_best, i_pi_best]`. (3) CDF truncation fix: appended `(pooled_max, 1.0)` final point to the Explorer (current) and Best-fit (algorithm) traces in both files; removed the `_x_candidates` block + hardcoded `xaxis_range = [0, _xmax_cdf*1.05]` so plotly autofocuses from data extents. Diagnosed two reported anomalies: (i) the "19 of 25 binaries at f_bin = 0.7" complaint was a stale mock — the slider was at 0.75/0.76/0.77 when the user clicked Generate, then moved to 0.70 without regenerating. The deterministic code at `app/bc/validation.py:222` produces exactly `int(round(25 × 0.70)) = 18` across all seeds. (ii) The banded 2D heatmap at fixed σ in the f_bin × π plane: per-set `n_bin = round(25 × f_bin)` is a step function of f_bin. With 99 f_bin grid points, the integer n_bin only resolves to ~26 distinct values; n_sets averaging is so effective via the LLN that the only structure left across nearby f_bin cells IS the integer quantisation, producing horizontal bands ~0.04 wide. n_sets is NOT being ignored — it is working too well, smoothing the orbital noise so completely that quantisation becomes the dominant cell-to-cell signal. Pending decision: switch from per-set deterministic n_bin to *aggregate-deterministic* floor/ceil distribution (`total_n_bin = round(n_sets × 25 × f_bin)` distributed across sets as floor or ceil) to restore a smooth f_bin axis on heatmaps without losing the exact-aggregate-fraction guarantee.

**Key results:**
- Explorer logL now agrees with grid logL bit-for-bit at grid nodes — achieved by reusing `simulate_delta_rv_cadence_aware` with `seed = seed_base + idx_of_nearest_cell`, deterministic `n_bin`, the same `n_sets`, and the same `multinomial_log_likelihood` against `lk_be`.
- The user can now sweep σ_single, f_bin, π, logP_max, and N_sets in real time on the Model Explorer, with a cached re-simulation on every slider change and a CDF that auto-extends across the union of mock + simulated extents.
- The horizontal-band artifact on f_bin × π heatmaps is now diagnosed (integer-quantised n_bin under aggressive n_sets averaging) — the fix is not yet committed but the design path is clear.
- Three-stage binarity workflow is now a written memo (`memory/project_three_stage_binarity_strategy.md`) and the open error-propagation question is flagged for explicit user resolution before Stage 2 is implemented.

**Methodology notes for paper:**
- **Three-stage binarity workflow.** Stage 1 (bias correction without assuming f_bin or a threshold) yields the best period model and intrinsic scatter parameters; Stage 2 (cadence-free bulk simulation at the Stage-1 winner) yields a parametric *binary RV distribution* — specifically the marginal density of ΔRV conditional on the system being a binary; Stage 3 (the RV Modeling tool) ingests the Stage-2 distribution and fits the *observed* f_bin, threshold, and (optionally) σ_single. The deliverable advantage over the historical single-shot bias correction is that the binary RV shape becomes a *derived* quantity rather than an assumed power-law, and the threshold becomes a *fitted* parameter rather than a fixed `45.5` km s⁻¹. Open methodological question: the error-propagation framing — distribution-of-distributions vs. scatter-of-PDF-parameters across Stage-1 posterior draws — should be decided before Stage 2 is implemented so the Stage-3 fit has a consistent uncertainty budget.
- **Explorer / grid logL consistency contract.** When an interactive explorer recomputes a quantity (logL, CDF, detection fraction) at the same parameters as the grid scorer, the contract is: same RNG seed (`seed_base + idx_cell` for the nearest grid cell), same `n_sets`, same deterministic `n_bin = round(N × f_bin)`, same likelihood-bin edges, same physics config (full `BinaryParameterConfig`, not a partial reconstruction). Any deviation produces a "the algorithm is wrong" complaint from the user — the symptom is not a bug in the algorithm, it is a re-simulation that lives on a slightly different physical surface.
- **Deterministic n_bin is the right convention for validation but the wrong convention for grid heatmaps.** At small N (25 stars), per-set Bernoulli `n_bin` introduces ±2σ noise that drowns the signal in the f_bin × π plane. At large `n_sets` (≥ 1000), per-set deterministic `n_bin` introduces a 1/N step in the f_bin axis (4 % per integer) that LLN-averaging cannot smooth. The principled fix is aggregate-deterministic distribution: `total_n_bin = round(n_sets × N × f_bin)` distributed across sets as `floor(total / n_sets)` or `ceil(total / n_sets)` so the *aggregate* fraction is exact while individual sets vary by ±1.
...
- Defer the σ_single-staging decision until the Tab 5 (`:8502`) results land; if recovery is clean, σ_single moves to Stage 1, otherwise it stays in Stage 3.
- Error-propagation framing left open — flagged for raising rather than silently deciding.
- Explorer logL must equal grid logL bit-for-bit at grid nodes — this is now a hard contract enforced by the cached wrapper + reused helper.
- Auto-update beats Run button when the computation is cache-fast — the Run-button gate from batch 1 was removed in batch 2 after the cached wrapper proved fast enough.
- `n_sets` is now user-editable inline (not in expander, no max constraint) — matches user-preference rules in `MEMORY.md` ("no min/max constraints, prefer `number_input` over sliders, never hide plot controls in expanders").

**Bugs found and fixed:**
- Bernoulli vs deterministic `n_bin` in two simulators (`simulate_delta_rv_sample`, `simulate_delta_rv_cadence_aware`) was producing per-cell Monte-Carlo binary-count noise that masqueraded as a parameter-recovery deficiency. Now both paths produce exactly `round(N × f_bin)` binaries per set. Not greppable as a code pattern (the two `rng.random(N) < f_bin` lines were already correct under the previous Bernoulli convention) but the new convention is documented in the simulator docstrings and `feedback_mock_deterministic.md`.
- Stale mock observation in the validation tab UI: the slider was moved after Generate without regenerating. Considered (but not yet shipped) a "(parameters changed since last generate — click Generate again)" caption next to the f_bin slider when `current_params ≠ session_state[mock_params_key]`.
- CDF truncation at `_xmax_cdf * 1.05` was producing a visually misleading "the algorithm stops where the data stops" artefact. Removed the hardcoded x-range; plotly now autofocuses from the union of mock + simulated extents.

**Open questions:**
- Aggregate-deterministic floor/ceil distribution for the heatmap-banding fix — awaiting user "ship it" or alternative.
- Should we add a "(parameters changed since last generate)" caption to the f_bin slider when the cached mock and current parameters disagree? Offered but not yet confirmed.
- Stage 1 → Stage 2 error propagation framing — needs a deliberate choice before Stage 2 is implemented.
- Five validation runs queued overnight: does the bias-correction sim recover σ_single when it is a free parameter (Tab 5 on `:8502`)? Does the recovery hold at f_bin extremes when σ_single is pinned (Tabs 1–2)? How sensitive is the recovery to bin placement near the steep CDF slope (Tabs 3–4)?

---

### 2026-04-30 (afternoon) — logP_max fixed-value input + corner-plot staircase diagnosis + single-star RV inspection plot

**What was done:**
- **logP_max scan: fixed-value input for the OFF state.** Added a `logP_max (fixed)` `st.number_input` to `_render_logPmax_scan` (`app/bc/params.py`) that appears when the scan toggle is OFF. Persists under `[sec, 'logPmax_fixed']` via `sm.save` on change; the default seeds from the existing `f'{p}_logP_max'` session state on first render. Modified the Dsilva and Langer cadence tabs (`app/bc/cadence.py`) to rebuild `_bin_cfg = dataclasses.replace(_bin_cfg, logP_max=float(_*_logPmax_vals[0]))` after the scan expander returns, so the typed fixed value flows into the cadence-aware simulation. Did NOT touch `runners_cadence.py` (still `# WORKING — do not change`). The new widget uses key `f'{p}_logPmax_fixed'` rather than reusing the orbital section's `f'{p}_logP_max'` because Streamlit forbids duplicate keys; decoupling lets the scan expander act as the single source of truth for the simulation when the scan is OFF without sync gymnastics. /error-check returned PASS in all 5 phases. User confirmed with "nice."
- **Corner-plot f_bin staircase diagnosis (no code change).** User reported step-like discontinuities on the f_bin axis of the (log₁₀ P_max, f_bin) and (σ_single, f_bin) corner-plot heatmaps despite f_bin being the *finer* grid (~99 points) than log₁₀(P_max) (~25 points). Root cause: `n_bin_per_set = int(round(n_stars_per_set * float(f_bin)))` at `wr_bias_simulation.py:937` (also line 716 for `simulate_delta_rv_sample`). With N ≈ 25 and 99 f_bin values, the 99 values collapse to ~26 unique integer n_bin → 4 adjacent f_bin grid points produce identical likelihoods → flat plateaus. Within-plateau wiggles are independent MC noise per cell; the relationship is `wiggle ∝ 1/√n_sets`, but `n_sets` does NOT remove the staircase — increasing it makes the steps *sharper*, not smoother. Three fix options were written into `~/.claude/plans/look-at-this-corner-logical-liskov.md`: (A) Bernoulli draw inside the MC loop; (B) analytic marginalisation Σ Binomial(k; N, f_bin) × L_k over per-k pre-computed CDFs; (C) cap `fbin_steps` at N+1. User chose **no code change** — the bias is bounded by 1/N ≈ 0.04 on f_bin, which matches the precision a 25-star sample can support, so the artefact is statistically defensible. Saved `memory/project_corner_staircase_known.md` so future sessions do not re-diagnose the same artefact.
- **Single-star RV / σ / ΔRV peak-to-peak inspection plot.** Built `scripts/plot_single_star_rvs.py` — loads observed RVs via `pipeline.load_observations.load_observed_delta_rvs(settings, ObservationManager())`, filters `is_binary is False & N ≥ 2`, sorts by σ ascending, plots per-epoch RV scatter with error bars + ⟨RV⟩ tick + σ-diamonds blended onto the *left* RV axis (same units as RVs). Marks the lowest σ (Brey 90a, 1.02 ± 0.68 km/s) and the largest σ (Brey 83, 11.27 ± 1.59 km/s). Implemented `std_uncertainty(rv, rv_err)` propagating per-epoch RV errors through the sample std via the closed-form Gaussian formula `err(s) = √(Σ d_i² σ_i²) / (s (N − 1))` (derived from `∂s/∂x_i = (x_i − x̄) / [s (N − 1)]`). Computed an inverse-variance weighted mean σ across the 15 single stars: **4.49 ± 0.17 km s⁻¹** (vs. unweighted 5.19 ± 0.69 km s⁻¹, population scatter 2.67 km s⁻¹). The right axis was repurposed for ΔRV peak-to-peak triangles with `err(ΔRV_pp) = √(err_max² + err_min²)`, recoloured violet to visually pair with the violet triangles. Output: `plots/single_star_rvs.html` (interactive). PNG export skipped — `kaleido` not installed in `guyenv`.

**Key results:**
- The simulation now uses the typed `logP_max` value when the scan is OFF — previously the orbital-section value was being passed through unchanged regardless of the fixed-value typed in the scan expander, which is a silent UX trap.
- The corner-plot staircase artefact is now formally diagnosed (1/N integer-quantisation in `n_bin`) and accepted as a statistically defensible 1/N ≈ 0.04 bias on f_bin. Three concrete fix options are recorded in the plan file if this needs to be revisited for the paper.
- Inverse-variance weighted single-star σ estimate: **4.49 ± 0.17 km s⁻¹** (formal propagated error), with a population scatter of 2.67 km s⁻¹ across 15 single stars. The propagated 0.17 km s⁻¹ reflects only measurement uncertainty; the 2.67 km s⁻¹ scatter is the more honest "uncertainty on a typical single-star σ". This becomes a candidate σ_single prior for the bias-correction simulator.

**Methodology notes for paper:**
- **Single-star σ as a candidate σ_single prior.** The inverse-variance weighted mean of the per-star sample standard deviations across the 15 single stars in the LMC sample is 4.49 ± 0.17 km s⁻¹ (formal propagated measurement-error uncertainty). The unweighted mean is 5.19 ± 0.69 km s⁻¹ and the star-to-star population scatter is 2.67 km s⁻¹. The weighted mean is biased low by stars with many epochs and small per-epoch σ; the unweighted mean is biased low by the smallest sample sizes; the population scatter is the right uncertainty if σ_single is interpreted as "the typical scatter a randomly drawn single WR star would exhibit". For the bias-correction prior, the population scatter should dominate the uncertainty budget, not the formal propagated error.
- **f_bin grid resolution and the 1/N bias.** Per-set deterministic `n_bin = round(N × f_bin)` with N = 25 limits the achievable f_bin resolution to ~1/N ≈ 0.04. Grid scans over f_bin at finer resolution than this produce a staircase artefact in the likelihood that increasing `n_sets` does NOT smooth (LLN-averaging makes the integer steps *sharper*, not softer). For the paper, either (i) accept the 1/N bias as the inherent precision of a 25-star sample, (ii) use Bernoulli per-set `n_bin` (introduces ±2σ MC noise but no integer steps), or (iii) use aggregate-deterministic distribution (`total_n_bin = round(n_sets × N × f_bin)` distributed across sets as floor / ceil). Option (iii) is the principled fix; options (i) and (ii) are the principled documentation-only choices.
- **Closed-form sample-std error propagation.** When per-epoch RV errors are known, the sample standard deviation `s = √(Σ (x_i − x̄)² / (N − 1))` has a Gaussian-propagated uncertainty `err(s) = √(Σ d_i² σ_i²) / (s (N − 1))` where `d_i = x_i − x̄`. This is exact for independent Gaussian-distributed RVs and is the right closed-form alternative to bootstrap or the asymptotic `s / √(2(N − 1))` estimator. The closed form was used in `scripts/plot_single_star_rvs.py` and is the appropriate choice when the RV measurement-error model is trustworthy at face value.

**Decisions:**
- Override `_bin_cfg.logP_max` upstream of the runner (in `cadence.py`) rather than modifying the runner — safer, since `runners_cadence.py` is `# WORKING`-flagged, and works identically for scan-ON (the first scan value is used as a placeholder; the runner overrides it per-iteration anyway).
- The corner-plot f_bin staircase is **accepted, not fixed**. The 1/N ≈ 0.04 bias matches the precision a 25-star sample can support, so the artefact is statistically defensible. Marked `memory/project_corner_staircase_known.md` as "do NOT re-investigate".
- The single-star RV plot uses the closed-form Gaussian propagation rather than bootstrap or the asymptotic `s / √(2(N − 1))` estimator — user explicitly asked for "error propagation", so the analytic version is the right interpretation.
- Not delegated to coder agent for the single-star plot script — task was a small standalone script in `scripts/`, not a paper or app/page edit, and user did not flag this as a coder-only domain. Edge-case for `feedback_coder_does_edits.md`; flagged in the daily log in case the user wants the rule extended.

**Bugs found and fixed:**
- The `logP_max` typed in the scan expander when the scan is OFF was not flowing into the simulation — the orbital section's `logP_max` was being passed through unchanged. Now overridden via `dataclasses.replace(_bin_cfg, logP_max=float(_*_logPmax_vals[0]))` after the expander returns. Not added to `COMMON_ERRORS.md` — UX bug rather than a greppable code pattern.
- `**PLOTLY_THEME` keyword collision when spreading `axis_kw` and re-specifying `showgrid=False` for `yaxis2` in the single-star plot script — already covered by **E018**. Same root cause, same fix: use a dict literal `{**base, 'key': override}` or explicit non-spread kwargs.

**Open questions:**
- **σ_intrinsic vs σ_measured.** The 4.49 ± 0.17 km s⁻¹ weighted mean is a candidate σ_single estimate but only if measurement-error-dominated; the 2.67 km s⁻¹ star-to-star scatter argues against that. Worth raising in the next bias-correction iteration whether the prior should be the formal weighted mean or the population scatter.
- **PNG export pipeline.** `kaleido` not installed in `guyenv`; HTML-only output is acceptable for inspection but blocks paper-ready PNG export. User to decide whether to install `kaleido` or to convert the plot via a screenshot at the figure-locking phase.
- **Aggregate-deterministic n_bin distribution** for the f_bin × π heatmap banding fix — still pending user "ship it" or alternative; rolled forward from the morning's session.

---

### 2026-05-04 — Bias-correction CDF panel enrichment + Likelihood Model Explorer Reset-to-best fix

**What was done:**
- **Likelihood Model Explorer Reset-to-best — joint argmax + slider quantisation bypass.** Two distinct bugs in `_render_lk_model_explorer()` (`app/bc/render_lk_explorer.py`) caused the Current Explorer logL to disagree with the stored Global best logL after clicking Reset-to-best. (a) Slider defaults came from a 2D-marginalized argmax rather than the joint argmax of the full N-D `logL_raw` cube. (b) When σ_single or logP_max wasn't scanned (size-1 grid), Explorer fell back to `sigma_meas` (measurement noise) instead of the grid's fixed value (`sigma_grid[0]` / `bin_cfg.logP_max`). Round 1 fix moved the joint-argmax decomposition before slider rendering, used `_bf_*` values as slider defaults, fixed the non-scanned-axis fallbacks, and rewrote the caption to show joint-argmax + `(fixed)` annotations for every dimensionality (4D, 3D σ-only, 4D-degenerate-σ for lp-only, 2D). Round 2 fix addressed Streamlit's silent slider quantisation: `st.slider(label, min, max, key=k)` for floats defaults `step=(max-min)/100` and snaps `session_state[k]` to the nearest tick before returning, so even setting the precise `_bf_fbin = 0.151428` rounded to `0.15` for the simulator. Workaround detects "first render after Reset" via a `_last_rc` tracker, then overrides `me_fb / me_x / me_sig / me_logPmax / _eff_logPmax` with the exact `_bf_*` floats post-widget-render (only on the Reset render — subsequent slider movements pass through untouched, per the user's explicit "no auto-snap-to-grid on every drag" preference). Quantisation pattern documented as new COMMON_ERRORS entry **E045** (`st.slider` with float bounds quantises value to implicit step `(max-min)/100`).
- **Bias-correction CDF Comparison panel enrichment (5 chunks, 6 files).** Added three orthogonal information layers to the live "CDF Comparison: Observed vs Best-Fit Models" panel (both Dsilva via `render_shared.py` and Langer via `render_shared_langer.py`): (i) per-star truth dots on the BLACK observed CDF — red=single, green=binary circles, sourced from `validation_io.load_per_star_truth(result)` for validation runs and from the existing `obs_detail['is_binary']` for real-obs runs (no recomputation of the detection criterion in plot code); (ii) per-rank median markers (squares) and mean markers (triangle-up) overlaid on each method's best-fit step line, snapped to the step y via `np.searchsorted(step_x, dot_x, side='right') - 1` so they ride exactly on the hv-step; (iii) per-rank colour encodes MC binary fraction via Plotly's native `marker.colorscale=[[0, _CLR_SINGLE], [1, _CLR_BINARY]]`, with a single colorbar (`MC binary fraction`, ticks 0% / 50% / 100%) emitted on the first rank-marker trace via a tracking flag dict. Three phantom legend traces (off-canvas singletons) communicate the marker-shape convention without exploding the per-method legend. To deliver the per-rank arrays, four new `best_*` keys are threaded from `wr_bias_simulation.simulate_delta_rv_cadence_aware` through `_single_grid_task_cadence_aware` (now an 11-tuple), `_process_result`, `run_bias_grid_cadence_aware`, persisted by `runners_cadence.py`, and exposed through a new `CDFBandResult` NamedTuple returned by `_me_cdf_band` and `_me_cdf_band_langer`. Diagnostic step during dev: silent `except: pass` in chunk wrappers replaced with `st.error(traceback)` / `st.warning(traceback)` to surface failures inline; will revert to silent only after fixture-verified visual approval.
- **Two existing learnings extended in `.claude/references/learnings.md`** rather than adding new entries: (i) "`result['settings']` is JSON string OR 0-d ndarray" — the `np.savez` round-trip can return either form; fix detects `isinstance(_settings_raw, np.ndarray)` first, calls `.item()` to unwrap, then guards with `isinstance(_, dict)` before falling back to settings manager (mirrored in Dsilva and Langer). (ii) "Silent `except: pass` is a trap" — generalised: any new fallback path needs `st.warning(traceback)` during dev; revert to silent only after fixture verification.
- **Plan-mode subagent harness inheritance noted but unresolved.** Subagents spawned during the morning session received a "Plan mode is active" system reminder despite the orchestrator session being out of plan mode and despite no settings.json/agent definition referencing plan mode. Workaround attempted (added "Plan-mode policy" exception clause to `.claude/agents/coder.md` so the coder skips plan mode when invoked with an approved plan reference); harness reminder still won, so the user opted to restart the conversation. Documented for future investigation.

**Key results:**
- Joint-argmax + non-scanned-axis-fallback pieces of the Reset-to-best fix verified by `scripts/test_render.py` 5/5 PASS and smoke-tested on a real Case-C grid `(15, 1, 49, 50)` — confirms the joint argmax decomposition is bit-exact across all four dimensionality cases. Slider-quantisation override is implemented but pending visual confirmation in the next session.
- CDF panel enrichment delivers three orthogonal information channels (per-star truth, per-rank median, per-rank mean) without obscuring the existing dashed median + 16/84 percentile band. Architecture deliberately separates LIVE renderers (`render_shared.py`, `render_shared_langer.py`) from the dead copy at `app/bc/analysis.py:325-580` (call site commented out at `subtabs.py:90`), avoiding double-edits.

**Methodology notes for paper:**
- **Per-rank decomposition of the model CDF.** The grid worker now persists per-rank median ΔRV and binary-fraction arrays (in addition to the legacy 16/84 percentile band of pooled ΔRV). The per-rank fraction encodes the MC variability of the binary count at fixed (f_bin, π, σ_single, logP_max), independently of the per-set deterministic `n_bin = round(N × f_bin)` constraint. For the paper, this becomes the visual evidence that the 1/N quantisation in f_bin (documented in the 2026-04-30 afternoon entry) is bounded by the integer step in n_bin, while orbital-noise variability is dominated by which N stars are drawn from the Galactic period/eccentricity distribution.
- **Streamlit float-slider quantisation as a silent precision trap.** When seed-reuse simulations require bit-exact parameter pass-through, the implicit `step=(max-min)/100` quantisation amplifies into observable score divergence (~0.04 in logL for the Reset-to-best case). Generalisable rule: any UI control that feeds a deterministic-seeded simulation must either (a) pass `step=` matching the grid spacing, or (b) maintain an off-widget reference to the precise float and treat the slider as display-only. Documented as E045 for future code review.

**Decisions:**
- LEFT bins control on the bias-correction page = CDF visual only; RIGHT bins control = likelihood score only. Caption added at `app/bc/params.py:446,453` to make this scope explicit; no behaviour change. (Ambiguity diagnosed in the morning session; behaviour was already correct, only the UI was unclear.)
- For observed-star binary labels on the CDF: reuse the already-classified `obs_detail['is_binary']` from `cached_load_observed_delta_rvs` rather than recomputing the detection criterion in plot code (single-source-of-truth convention).
- For simulated-star markers: per-rank median ΔRV (placement) + fraction-coded continuous colour (information). Each rank's binary status is a distribution across draws — colour gradient preserves the distribution rather than picking one representative draw.
- Marker-shape legend convention: SHAPE encodes which line (median = square, mean = triangle), COLOUR encodes MC binary fraction. Two orthogonal channels → no ambiguity even at high marker density. Phantom legend entries (off-canvas singletons) used instead of per-method legend explosion.
- Reset-to-best targets joint argmax of full N-D `logL_raw`, NOT 2D-marginalized argmax. Caption + Reset target + "Global best logL" metric are now internally consistent.
- Non-scanned axes use grid-fixed values from the save (`sigma_grid[0]` / `bin_cfg.logP_max`), never `sigma_meas`. Caption shows them with `(fixed)` annotation.
...
**Bugs found and fixed:**
- E045 (`st.slider` with float bounds quantises value to implicit step `(max-min)/100`): added with full Bad/Fix/Why/Found-in/Prevention sections. Fixed in `app/bc/render_lk_explorer.py` via post-widget-render override on Reset.
- 2D-marginalized argmax used as Reset-to-best target instead of joint argmax of N-D `logL_raw` cube — caused caption to show one cell while sliders moved to another. Fixed by relocating the joint-argmax decomposition block above slider rendering.
- Non-scanned σ/lp axes used `sigma_meas` instead of grid-fixed value as slider default — caused Reset to land on the wrong σ/lp slice. Fixed with explicit `if sigma_grid.size == 1: _bf_sig = float(sigma_grid[0])` (same for logP_max).
- 0-d numpy object array variant of E043 (`AttributeError: 'numpy.ndarray' object has no attribute 'get'` from `app/shared.py:settings_hash`) — `result.get('settings')` returns a 0-d ndarray after `np.savez` round-trip, and the existing `or get_settings_manager().load()` fallback didn't fire because the ndarray is truthy. Fixed by detecting `isinstance(_settings_raw, np.ndarray)` first, unwrapping with `.item()`, then guarding with `isinstance(_, dict)` before falling back. E043 learning extended (no new E entry).
- Silent `except: pass` in CDF panel chunk wrappers swallowed the actual exception and made the panel appear blank in Round 1. Replaced with `st.error(traceback)` / `st.warning(traceback)` for the duration of the visual-debug cycle; will revert to silent only after fixture-verified approval.

**Open questions:**
- **PENDING visual approval (next session)**: load BC page on a real-obs run and verify the 5 enrichment features (per-star truth dots on observed CDF, squares riding median lines, triangles riding mean lines, "MC binary fraction" colorbar with red/gray/green at 0%/50%/100%, three phantom legend entries). If any fail, the surfaced `st.warning` will tell us why. After approval: revert `st.error` and `st.warning` diagnostics back to silent (`pass`).
- **PENDING visual confirmation (next session)**: round-2 Reset-to-best fix may not have closed the gap entirely (user reported "i think its still the same" but ended conversation before providing screenshot). Next theory to check: cadence library precision through save/load round-trip, or whether `seed_base` is actually persisted in user's grid `.npz` (Explorer falls back to 1234 if absent).
- Subagent plan-mode harness injection — root cause unidentified; config files don't reference plan mode but spawned coders consistently received "Plan mode is active" system reminders. Likely VSCode-extension state inheritance from session start. Worked around by restarting the conversation; not blocking but worth investigating later if it recurs.
- `/error-check` 5/5 PASS reported by coder agent did not catch the visual regression in Round 1 — `scripts/test_render.py` exercises the renderers in bare mode but doesn't verify which traces are present in the final figure. No new learning needed (covered by existing "NEVER self-approve plots" rule); the orchestrator (Claude) trusted the agent's report without spot-checking the actual figure.

---

### 2026-05-07 — Validation corner-plot truth markers, π explainer PDF, smooth pooled-empirical CDFs, mock-data distribution verification

**What was done:**
- **Validation corner plots — green dashed "true input" markers (TODO 200).** The validation tab persists user-chosen mock-input parameters (`true_fbin`, `true_pi`, `true_sigma`, `true_logPmax`) into the result npz with `is_validation=True`. Diagonal 1D marginals now display a **green dashed vertical line** (`#16A34A`, dash 1.5 px, annotation `True input` top-left so it doesn't collide with the existing red top-right `Joint argmax`). Lower-triangle 2D heatmaps display a **green × marker** at `(true_x, true_y)` overlaid on the gold-star joint-argmax marker. The patch had to land in **three** files because the Likelihood scoring tab has its own duplicated corner-plot implementation that bypasses the shared helper: `app/bc/corner_plots.py:_render_corner_plot` (used by K-S / CvM / weighted-CvM tabs), `app/bc/render_lk_fit.py:_render_lk_corner_plot` (Dsilva Likelihood tab), and `app/bc/render_lk_fit_langer.py:_render_lk_corner_plot` (Langer Likelihood tab). Each file received the same 4-edit pattern: `_add_1d_posterior` gains `true_value=None` + green `add_vline`; `_add_2d_heatmap` gains `true_x=None, true_y=None` + green × Scatter; `_render_*_corner_plot` gains a `_truth_keys` dict + `_is_validation` flag + `_truth_for(axis_name)` helper gated on `result.get('is_validation', False)`; caption gains a conditional truth-marker clause only when validation mode is active AND at least one `true_*` value is finite. Non-validation runs and legacy npz silently skip the green elements (`np.isfinite` guard).
- **π parameter explainer PDF (TODO 201).** Built `scripts/make_powerlaw_explainer.py` — single-script generator that imports the production `sample_logP_powerlaw` from `wr_bias_simulation.py:372-403` and emits a self-contained 6-page PDF via `matplotlib.backends.backend_pdf.PdfPages`. The pages walk the slider → `generate_mock_observations` → `simulate_delta_rv_cadence_aware(pi=…)` → `sample_logP` → `sample_logP_powerlaw` chain, derive the `(π+1)` exponent in the inverse-CDF formula, plot prior PDFs and CDFs for π ∈ {−2, −1, −0.5, 0, 1, 2}, validate the inverse-CDF against 200k samples (perfect overlap), and show the period-in-days view contrasting π=0 (Öpik) with π=−1 (1/log P). Output: `docs/sample_logP_powerlaw_explained.pdf`.
- **Smooth pooled-empirical CDFs across all simulated-star plots (TODO 202).** Three-round iteration on the live "CDF Comparison" panel converged on a pooled empirical recipe extracted into `bc.helpers.smooth_pooled_cdf(pooled, n_sets, n_fine=500)`: returns `(sorted_pool, y_pool, x_fine, lo_fine, hi_fine)` where the dashed line is the empirical CDF over all `n_sets × n_stars` ΔRVs and the band is the 16/84 percentile across draws computed on a 500-point linspace via `np.searchsorted`. Applied to **5 plot sites**: `render_shared.py` and `render_shared_langer.py` (mock-vs-best-fit), `render_lk_fit.py` and `render_lk_fit_langer.py` (Likelihood Best-Fit), and `likelihood_viz.py` (alternate Likelihood Best-Fit panel). Brought the Langer mock-vs-best-fit twin to parity with the powerlaw twin (dropped mean dotted CDF + triangle markers + phantom legend, bumped height 400 → 600). For the Likelihood Best-Fit panels, also switched the **observed** CDF to a true empirical step (`np.sort(obs_drv)`, `shape='hv'`) so it matches the canonical mock-observation step in the other panels.
- **Mock-data distribution verification (TODO 203).** Verified parallel-Claude's claim that the Validation tab's mock-data generator pulls from a different distribution than the sliders. Created `scripts/test_mock_distribution.py` with 5 orthogonal cases (singles σ_meas=0, two-source convolution, deterministic n_bin, f_bin extreme dispatch, fixed-vs-Log-normal shape). All 5 pass → claim REFUTED. Recovered σ̂ matches input σ_single within 0.12 % at σ_meas=0 and matches `√(σ_single² + σ_meas²)` within 0.06 % at σ_meas=5. Two-source observational model in `app/bc/validation.py:152-165` is implemented correctly. Then misread the user's "I want a new GUI for verifying mock data" as an ask for a generic test-runner page — built `app/pages/13_tests_verification.py` + `app/pages/_tests_verification_helpers.py` (auto-discovers `scripts/test_*.py`, displays source, runs via threaded `subprocess.Popen` with `st.fragment(run_every=0.5)` live streaming). User clarified: "a new GUI for X" means a dedicated interactive inspector with X-specific controls and live plots, not a generic CLI wrapper. Saved as new feedback memory `feedback_new_gui_means_dedicated_interactive.md`. Tests page is left on disk per user's instruction; next session will build the actual inspector.

**Key results:**
- Truth-marker patch produces 12× `#16A34A` colour and 3× `True input` annotations in a real validation `.npz` (`fbT0.70_piT-3.00_sigT2.0_logPT3.50_seed42`); `/error-check` 9/9 PASS twice (once per round).
- Smooth pooled empirical CDF: with ~25 000 pooled ΔRVs the steps are 1/25k wide → naturally invisible (no interpolation tricks). 500-point linspace x-grid for the band gives smooth-looking output without per-draw evaluation at 25k pooled x-points.
- Mock-data verification: recovered σ̂ within 0.12 % of input at σ_meas=0; within 0.06 % of `√(σ_single² + σ_meas²)` at σ_meas=5; deterministic `n_bin = round(N × f_bin)` exact across 8 fbin values × 5 seeds.

**Methodology notes for paper:**
- **Pooled empirical CDF as the single visual primitive for simulated-star CDFs.** With `n_sets × n_stars` samples (typical 1000 × 25 = 25 000), an empirical CDF is automatically smooth — no kernel density / spline interpolation needed. The band that quantifies set-to-set scatter is the 16/84 percentile across per-draw empirical CDFs evaluated on a fine linspace x-grid (the per-draw CDFs are themselves step functions; the band envelope is interpretable as "what fraction of MC draws have CDF ≤ y at this ΔRV"). Observed-star CDF stays as a true empirical step (`shape='hv'`) because N_obs ~ 25 makes individual steps visible and meaningful.
- **π=−1 prior is uniform in `ln(log P)`, NOT uniform in P.** The prior `p(x) ∝ 1/x` with `x = log P` is scale-invariant on the log-period axis: `p(log P) ∝ 1/log P` integrates to a logarithm in `log P`, so the implied period distribution `p(P) = p(log P) / (P ln 10)` falls off as `1/(P × log P)`. Caught when inspecting page 6 of the explainer PDF; rewrote the caption.
- **Two-source observational model.** `_sample_delta_rv_mock` and `generate_mock_observations_detail` in `app/bc/validation.py` add `σ_meas` (measurement noise) on top of the `σ_single` intrinsic scatter for single stars and on top of the orbital-Kepler RVs for binaries; `σ_meas_binary` defaults to `σ_meas_single` unless explicitly set. The deterministic `n_bin = int(round(N × f_bin))` (with `rng.choice` picking which stars are binary) ensures input parameters land in the data verbatim across seeds.

**Decisions:**
- Truth markers gated on `result.get('is_validation', False)` AND finite `true_*` value — non-validation runs and legacy npz silently skip the green elements (no errors, no caption change).
- Annotation `True input` placed top-left to avoid colliding with existing top-right `Joint argmax` annotation.
- WORKING-flagged comment banners in `render_lk_fit*.py` updated to `UPDATED 2026-05-06: added validation truth-line markers — user authorised` per project rule (user explicitly authorised the change → flag updated rather than removed).
- Pooled empirical CDF (sort all 25k ΔRVs, y = (k+1)/N) chosen over a fine-grid median CDF for the line. With ~25k points the steps are 1/25k wide and invisible — naturally smooth, no interpolation tricks needed.
- 500-point linspace x-grid for the band: enough resolution to look smooth, fast enough to render in Plotly.
- Shared helper `bc.helpers.smooth_pooled_cdf` rather than 5× inline duplication — single helper used 5 times is DRY, not bloat.
- Self-contained matplotlib `PdfPages` chosen over `pdflatex` for the explainer PDF — keeps the artefact regenerable with one command and no toolchain coupling.
- A&A-style rcParams (white bg, black text, serif, viridis-like palette) for the explainer figures — consistent with project plot rules.
- Tests page (`app/pages/13_tests_verification.py`) is the wrong shape for the user's "new GUI for verifying mock data" request — it is functionally clean but kept on disk pending decision; the actual feature work is to build a dedicated mock-data inspector next session.

**Bugs found and fixed:**
- E052 (`\bigl` / `\bigr` family unsupported by matplotlib mathtext, crashes at draw time) — added to COMMON_ERRORS.md with full Bad/Fix/Why/Found-in/Prevention sections. Found in `scripts/make_powerlaw_explainer.py:152` initial draft; fix uses plain `[`, `]` or `\left[ \ldots \right]`.
- E053 (`ax.text(..., wrap=True)` corrupts `$math$` segments — wrapping inserts newlines mid-formula) — added to COMMON_ERRORS.md. Found in `scripts/make_powerlaw_explainer.py` page-6 caption initial draft. Fix: pre-wrap at safe word boundaries treating each `$...$` as one atomic unit, join with `\n`, pass with `linespacing=1.5` instead of `wrap=True`.
- Round-1 truth-marker patch missed the duplicate `_render_lk_corner_plot` definitions in `render_lk_fit.py` and `render_lk_fit_langer.py` — green line never showed on user's Likelihood-tab screenshot. QA agent diagnosed via grep + figure inspection; round-2 fix applied the same 4-edit pattern to all three corner-plot implementations.
- π=−1 caption originally claimed "uniform in P" — scientifically incorrect; the prior `p(x) ∝ 1/x` with `x = log P` is uniform in `ln(log P)`, NOT in P. Caught when inspecting page 6 of the explainer PDF; rewrote the caption.

**Open questions:**
- **PENDING visual approval (next session)**: open Bias Correction → Validation tab → Likelihood scoring tab → expand corner plot → confirm green dashed vlines on each diagonal panel at the chosen truth values + green × on each 2D heatmap. Also test K-S / CvM / Weighted tabs (all should now show green markers via the shared `corner_plots.py` path).
- **PENDING visual approval (next session)**: 5 smooth-pooled-CDF panels (Dsilva + Langer twins for mock-vs-best-fit and Likelihood Best-Fit, plus alt Likelihood Best-Fit panel in `likelihood_viz.py`). Pending follow-ups: Model Explorer CDF in `app/bc/render_lk_explorer.py` still uses coarse visualization bin grid; "last CDF of Panel B" also still coarse — identify which file/panel.
- **NEXT-SESSION feature**: build the actual mock-data inspector GUI (sliders for σ_single, σ_meas, f_bin, error_model, true_logPmax → live `bc.validation.generate_mock_observations_detail` → per-epoch RV histogram vs theoretical N(0, √(σ_single² + σ_meas²)), is_binary count vs round(N·f_bin), ΔRV CDF, KS verdict overlay).
- Sub-agent plan-mode harness inheritance hit again across two sessions — coder/Write sub-agents repeatedly stalled in plan-mode despite parent being out. Worked around by direct orchestrator edits with user permission. Same issue logged 2026-05-04 — root cause unresolved; not blocking but worth investigating later.

---

### 2026-05-10 — Validation Model Explorer CDF smoothing + standalone `mock_inspector_app/` + q_range mismatch fix (E048 recurrence)

**What was done:**
- **Validation Model Explorer CDF — smooth pooled empirical + 16/84 percentile band (TODO 204).** Closed the follow-up flagged in TODO 202: the Model Explorer CDF on the Validation tab still rendered as a coarse step CDF with no uncertainty band despite running `n_sets` MC iterations. Reused the existing `smooth_pooled_cdf(pooled, n_sets, n_fine=500)` helper at `app/bc/helpers.py:118` (already used by `render_shared.py:637-659` for the validation tab's other CDF panel). The Explorer (current params, dashed red) and best-fit overlay (dotted red, when checkbox ticked) now both display a smooth pooled empirical CDF plus a 16/84 percentile band; original step trace kept as the empty-pool fallback (`if _scdf is None`). Three hunks per file (import + Explorer trace block + best-fit overlay block) applied to both `app/bc/render_lk_explorer.py:1124` (Dsilva) and `app/bc/render_lk_explorer_langer.py:698` (Langer). Reused the LOCAL `_hex_to_rgba` already defined at the top of each file instead of re-importing from `bc.helpers` — avoids shadowing/redefinition warning.
- **Standalone `mock_inspector_app/` Streamlit webapp (TODO 205).** Designed and built a NEW standalone Streamlit webapp (not a page in `app/`), launched on port 8503. Five files (~2200 lines): `__init__.py`, `settings.py`, `runner.py`, `inspector.py`, `app.py`. Imports from existing project (`bc.helpers`, `wr_bias_simulation`, `pipeline.load_observations`) but writes to its own `mock_inspector_app/settings.json`. Compares two pipelines side-by-side at the same params/seed: **Mock Data** (`rng.choice` for binary indices, the validation-tab path) vs **Model Explorer** (`rng.permutation`, the explorer's re-sim path). Both call the same inner `simulate_delta_rv_cadence_aware` → byte-identical orbital draws; only the binary-index assignment differs. Per pipeline: ΔRV CDF (smooth pooled empirical + 16/84 band via `smooth_pooled_cdf`), f_bin-vs-threshold curve, 3×3 orbital histograms grid (logP, e, q, K1, M1, M2, i_deg, ω_deg, T0_rad — matching the `render_shared_langer.py:1014` `render_orbital_histograms` convention). Iteration: 8 rounds converged on 3×3 overlay (Mock blue + Explorer red overlaid in each panel) + a single-run "🎯 Add single run" button with red/green per-star dots that accumulate across multiple seed clicks (per-run step lines collapsed to one legend entry via `legendgroup='single_runs'`); user-controlled "Std band bins" `number_input` above both panels for the f_bin 16/84 band. Multiprocessing experiment reverted: `mp.Pool(spawn)` blocks the Streamlit thread for 5–10 s on macOS; pure serial loop with inline 100-call progress throttle preferred.
- **Validation Model Explorer mock-vs-grid CDF mismatch — q_range fix (TODO 206, E048 recurrence).** User reported: Model Explorer dashed CDF (at user-set truth params) visibly fits the mock observation worse than the algorithm best-fit dotted CDF, even though those exact truth params generated the mock. After three false starts (logP_max plumbing, 1.0-logL truth-cell reproducibility gap, mock unreproducible from saved metadata), the smoking gun was found at `app/bc/render_validation.py:351-357`: the UI builds `BinaryParameterConfig(logP_min=0.15, logP_max=true_logPmax, period_model=period_model, e_model=e_model, e_max=0.9)` with only 5 fields explicitly set, leaving `q_range` at dataclass default `(0.1, 2.0)` per `wr_bias_simulation.py:318`. The grid uses the FULL `bin_cfg` from the orbital-params widget which is `q_range=(0.1, 4.0)` for this user's settings. Higher q ⇒ heavier companion ⇒ larger K1 ⇒ larger ΔRV. Mock samples a narrower ΔRV distribution than the grid evaluates against. Fix: build `_base_bin_cfg` from `settings['grid_cadence_dsilva']['orbital']` (Dsilva) or `settings['grid_cadence_langer']` (Langer) — same source `cadence.py` reads — then `dataclasses.replace(_base_bin_cfg, logP_min=0.15, logP_max=true_logPmax, period_model=period_model, e_model=e_model, e_max=0.9)` for the truth-specific overrides. Same root cause as **E048** (re-sim helper builds fresh `BinaryParameterConfig` and discards the user's bin_cfg) at a new call site; recurrence note appended to E048 in `COMMON_ERRORS.md`.

**Key results:**
- Validation Model Explorer CDF now smooth across both Dsilva and Langer; 16/84 percentile band tracks slider movement; render_test 5/5 PASS, all 6 downstream consumers re-import cleanly.
- Cross-tab mock divergence diagnosed (Mock Inspector vs Validation tab): root cause = σ_meas default 5.0 in new GUI vs 1.622 in validation tab (`extras.py:578`). σ_meas inflates each star's ΔRV directly because ΔRV is peak-to-peak of `orbital_signal + N(0, σ_meas)` per epoch — at σ_meas=5.0 vs 1.622, the noise floor of single-star ΔRVs jumps from ~3–7 km/s to ~9–20 km/s.
- q_range bug numerical confirmation: at truth (0.7, 3.0, 10.0, logP_max=3.5), mock smooth (q≤2) vs grid smooth (q≤4) differ by 0.05–0.09 in CDF across the entire ΔRV range — exactly the offset the user kept pointing at. Saved mock empirical CDF is consistent with q≤2 model (within 1σ for N=25); appears as a +2σ outlier vs the q≤4 model the Explorer plots.
- Codebase audit revealed FIVE different binary-index sampling methods across the validation/explorer/sim paths (`rng.choice`, two `rng.permutation` sites, two `rng.random < f_bin` Bernoulli sites at `wr_bias_simulation.py:1073` and `:2132`). The Bernoulli sites violate the `feedback_mock_deterministic.md` rule (n_bin should be exactly `round(N × f_bin)`, not Bernoulli) and are flagged as a follow-up.

**Methodology notes for paper:**
- **Pooled empirical CDF as the single visual primitive in the Model Explorer too.** Same recipe as TODO 202 now applied to the validation Model Explorer panel: with `n_sets × n_stars` samples (typical 1000 × 25 = 25 000) the empirical step is automatically smooth; the 16/84 percentile across per-draw empirical CDFs evaluated on a 500-point linspace is interpretable as "what fraction of MC draws have CDF ≤ y at this ΔRV". Original step trace retained as the empty-pool fallback so we never silently lose the line.
- **Result-dict completeness for re-sim helpers — generalised.** E048's lesson (persist `bin_cfg`/`period_model`/`cadence_weights` in the runner's result dict so re-sim helpers don't have to guess) extends to UI mock generators, not just to `_me_cdf_band` style helpers. Any helper that builds a `BinaryParameterConfig` from scratch must accept either the full dataclass or its `vars(...)` dict, then apply truth-specific overrides via `dataclasses.replace`. The orbital-params widget config is the single source of truth (`settings['grid_cadence_*']['orbital']`) — same dict `cadence.py` reads.
- **σ_meas affects ΔRV directly, not just an errorbar.** ΔRV = peak-to-peak of `orbital_signal + N(0, σ_meas)` per epoch. So changing σ_meas from 1.622 → 5.0 makes the noise floor of single-star ΔRVs jump from ~3–7 km/s to ~9–20 km/s. When troubleshooting cross-pipeline ΔRV divergences, σ_meas is the first thing to check.

**Decisions:**
- Validation Model Explorer band: 16/84 percentile (matches project standard — Panel B, render_shared marginal/grid CDFs, `feedback_honest_labels.md` 68 % HDI rule). Apply to both current AND best-fit overlay traces for visual consistency: current = dashed red @ 0.20 fill opacity, best-fit = dotted red @ 0.12 fill opacity (so the two bands compose without illegibility).
- Drop `shape='hv'` on the median line — that was the source of the staircase look. Smooth pool sort makes it natural.
- WORKING flag above `_render_lk_model_explorer` left intact: user explicitly authorised the change, so the FLAG WORKING CODE rule (#5) is satisfied.
- `mock_inspector_app/` is a NEW standalone webapp on port 8503, not a page in `app/`. No changes to existing project files (`app/**`, `wr_bias_simulation.py`, `pipeline/**`, `bc/**` all untouched per "don't touch working code" rule). Imports the project as a library; writes to its own `settings.json`.
- 3×3 grid uses `make_subplots(rows=3, cols=3)` despite the project's "no make_subplots" preference, because the existing bias-correction page (`render_shared_langer.py:1072`) uses the same pattern for the same use case. Project rule wasn't absolute — applies when subplots cause cross-figure interference.
- Multiprocessing reverted at user request after spawn overhead made the UI freeze at N_iter=5000. Spec was correct (byte-identical to serial, throttle to 100 callbacks max), but `mp.Pool(spawn)` blocks the Streamlit main thread for ~5–10 s during worker startup. No workaround short of `concurrent.futures.ProcessPoolExecutor` in a background thread + `st.fragment` polling, which is significant complexity for marginal gain.
...
- **q_range fix is one-sided (mock generator only)**. Do NOT modify the grid worker side — the grid is the source of truth for physics knobs; the mock just needs to inherit them. Override only truth-specific fields (`logP_min`, `logP_max`, `period_model`, `e_model`, `e_max`); everything else (q_range, mass_primary_*, langer_*, q_flipped) inherits from the user's orbital-params widget.

**Bugs found and fixed:**
- **E048 recurrence** at `app/bc/render_validation.py:351-357` — UI mock generator built fresh `BinaryParameterConfig` with only 5 fields, leaving `q_range` at default `(0.1, 2.0)` while the grid worker used `(0.1, 4.0)` from the orbital-params widget. Caused 0.05–0.09 systematic CDF offset between mock and Explorer dashed line; algorithm best-fit drifted from truth by 3.4 logL. E048 entry in `COMMON_ERRORS.md` extended with the 2026-05-10 recurrence note (same root cause, new call site — no new E-number minted).
- No new COMMON_ERRORS patterns from sessions 1 (smooth pooled CDF — feature addition, not bugfix) or 2 (mock_inspector_app — `mp.Pool(spawn)` blocking is architecture-specific not a greppable code pattern; σ_meas default mismatch is a defaults problem, also not greppable).

**Open questions:**
- **PENDING visual approval (tomorrow)**: Model Explorer CDF on the Validation tab — smooth dashed red + 16/84 band (Explorer current); dotted red + lighter band (best-fit overlay when checkbox ticked); both bands compose readably; Langer page identical.
- **PENDING visual approval (tomorrow)**: q_range fix — set truth + `q_range=(0.1, 4.0)`, Generate, verify the mock empirical CDF max ΔRV is now in typical range for q≤4 (often >200; fresh code at seed=42 gives max=244). Validation grid run should land Explorer-at-truth logL within single-mock noise of the algorithm's best.
- **PENDING decision**: σ_meas default in `mock_inspector_app/` — change 5.0 → 1.622 to match validation tab? Two options: (a) change default + leave knob, (b) keep at 5.0 with caption noting validation-tab default.
- **Does the q_range fix also resolve the ~1.0 logL stored-vs-recomputed truth-cell gap?** Likely yes (stored grid scored against a mock generated under a different model than the grid's own physics), but won't know until a fresh validation run is done. If it persists, separate investigation.
...
- **Bernoulli f_bin sites in `wr_bias_simulation.py:1073, :2132`** — violate the deterministic-mock rule. Not addressed; raised as follow-up.
...
**New learnings (3):**
1. **Don't dismiss a user-reported divergence after one incomplete bisect.** The first bisect script passed `sigma_meas=5.0` to both paths (comparing GUI defaults to GUI defaults) and missed the actual divergence. Before claiming "no divergence", trace the OTHER tab's actual UI defaults from its source code, don't assume. Added to `learnings.md` Context & Memory.
2. **σ_meas affects ΔRV directly** — not just an errorbar. ΔRV = peak-to-peak of `orbital_signal + N(0, σ_meas)` per epoch. When troubleshooting cross-pipeline ΔRV divergences, σ_meas is the first thing to check.
3. **When the user pushes back on "this is statistical noise", run the empirical comparison BEFORE re-presenting the argument.** Cost ~90 minutes today: presented a coherent statistical story (Wilks Δχ²=4.6 → 88 % joint confidence → "expected for 25-star draws") ~5 times in different framings as the user kept saying "no, 5 seeds all cluster, this isn't noise". User was right; the actual bug was a 5-line UI mock-config construction that any direct distribution-comparison test would have caught immediately. Added to `learnings.md` Data & Debugging at top of section. Generalisation of the existing "verify explore-agent hypotheses against the code before accepting them" rule — applies to OWN hypotheses too, not just sub-agents'.

---

### 2026-05-18 — Validation tab UX consolidation: Max ΔRV removal, truth-line fix, Model Explorer bin editor + snapshot overlays, mock stacking, Load-syncs-bins; first big code push to main since 2026-04-19

**What was done:**

- **Max ΔRV (km/s) cadence-bin control removed; auto-derived from observed ΔRVs (TODO #210).** The user-facing `number_input` in the Cadence-aware settings panel was coupling the non-adaptive bin grid's upper bound directly to the Mock Observation step CDF, truncating the rendered observation at 460 km/s while individual ΔRV points extended out to ~900 km/s (visible on the Model Explorer chart). Added `_auto_drv_max(obs_drv, bin_width, headroom_bins=5)` at `app/bc/params.py:442-452` that rounds `max(obs)` up to a bin-width multiple plus five headroom bins, falling back to 500 km/s when observations aren't loaded yet. Both `_render_cadence_adaptive_bins` non-adaptive branch (`params.py:482-495`) and `_cadence_run_and_results` (`cadence.py:1015-1028`) now call this helper so the displayed bin count matches the simulation bin grid exactly. Caption updated from `N bins` → `N bins · max ΔRV = X km/s (auto)` so the resolved range stays visible. The Mock Observation step CDF now extends to the full data range automatically.

- **Validation lane bug — truth fields missing from in-memory result on fresh runs (E054, TODO #209).** `_run_cadence_bg` mock-backend branch at `runners_cadence.py:672` was setting `result['is_validation'] = True` but persisting the four `true_*` truth fields to disk only (via `_vio.save_validation_result(..., _validation_truth, ...)`) — they were never attached to the in-memory `result` returned to the UI. Consequence: `corner_plots._truth_for(axis_name)` returned None on fresh runs because the key was missing; the green dashed truth lines vanished. The lines reappeared only after the user reloaded the same `.npz` from disk (because the disk reader merged the truth back). Fix: 5 added lines that mirror the disk metadata writes onto the in-memory dict — `result['true_fbin'] = float(_validation_truth.get('true_fbin', np.nan))` and the three siblings. Pattern recorded as **E054** in `COMMON_ERRORS.md` ("Validation lane persists metadata to disk but not to the in-memory `result` dict"). Generalisation of E048 / E051 / E048-recurrence: any background runner that uses a sibling save-helper with separate metadata args must mirror those writes onto the in-memory result before returning, otherwise the disk and memory shapes diverge and any consumer that reads the missing key sees None.

- **Model Explorer — tunable likelihood-bin editor + saved-attempts snapshot table + colored CDF overlays (TODO #211 + #212).** Implemented across four files: `params.py` (new `_render_explorer_lk_bin_config` helper, Threshold/Manual modes mirroring the global widget but persisting to a separate `explorer_likelihood_bin_config` namespace so the simulation's saved bins are never overridden), `helpers.py` (new `_SNAPSHOT_PALETTE` of 8 distinct colors with paired lighter shades), `render_lk_explorer.py` (bin editor wired above the parameter sliders at L1271, snapshot store keyed `f'{prefix}_lk_me_snapshots'`, 💾 Save / 🧹 Clear-all UI inserted between the CDF chart and the histograms/det-frac panels, per-row × delete via `st.columns(...)` rows, colored CDF traces + dashed median per snapshot inside the CDF figure assembly block, optional bin-edge vlines in matching colors when the "Show likelihood bin edges on CDF" checkbox is toggled), and `render_lk_explorer_langer.py` (1-for-1 mirrored twin with independent `_lk_me_langer_snapshots` store so DSilva and Langer snapshot stacks don't interfere). Snapshots are session-only — not persisted to disk per the user's explicit instruction. Two visual bugs found and fixed in a second iteration (TODO #212): (i) the first implementation stored `median_cdf` (binned step values at simulation bin edges `be`) which rendered visibly coarser than the live Explorer's `smooth_pooled_cdf(pooled_drv, _n_sets_me)` line — the user perceived this as "different n_sets"; (ii) the snapshot's "shadow" trace used `y=np.concatenate([_smy, np.zeros_like(_smy)[::-1]])` which filled from median curve down to y=0 instead of being a symmetric 16/84 band. Both fixed by capturing the full `smooth_pooled_cdf` 5-tuple `(_sp, _yp, _xf, _lo, _hi)` as tuples-of-floats at Save time and replaying with the same lo / hi / median pattern as the live Explorer trace at `render_lk_explorer.py:1633-1650`. Backward-compatible: legacy snapshots without the `smooth_cdf` key fall back to step rendering. The feature lets the user investigate whether their grid's logL is genuinely poor or whether bin choices are the cause: tweak bins → see logL update instantly (CDF curve is built on `be`, not `lk_be`, so only the score changes when bins move) → hit Save to freeze the attempt → compare logL values across attempts side-by-side.

- **Stack mock observations on CDF (TODO #213).** Each click of "Generate Mock Observations" on the Validation page now appends a compact snapshot to a per-page session-state stack `f'{p}_val_mock_stack'` instead of overwriting. Prior mocks render as thin dotted colored step CDFs on the Mock Observations Preview chart — no markers, only step lines, to keep the chart readable. Active (most-recent) mock still drives the recovery flow. A 🧹 Clear-stack button with a live counter sits next to the Generate button; a "📚 N mocks stacked …" caption appears when ≥2 entries are present. Snapshot stores ONLY `delta_rv` + `is_binary` + seed + truth params (no per-star RV/err arrays) so session-state stays small after 20+ stacked mocks. Colors cycle through `_SNAPSHOT_PALETTE`. The feature was a restoration: it existed in a prior uncommitted state but never made it into git (the validation page was first committed today in `faf92da`), so this is build-from-scratch.

- **Sync likelihood bins on Load via one-shot `is_loaded_result` session flag (TODO #214).** When loading a saved validation result from `mock_results/`, both bin widgets (simulation-params global at `_render_likelihood_bin_config` + Model Explorer at `_render_explorer_lk_bin_config`) now pre-fill with the saved run's `likelihood_bin_edges` automatically. Approach: Load handler in `render_validation.py:101` sets `f'{p}_is_loaded_result'=True`; both widgets check the flag at the top and override their session-state keys via the new `_infer_lk_bin_mode(bin_edges)` helper at `params.py:500-516` (recognises the canonical `dsilva_likelihood_bins(T)` schema `[0, T, 250, 650, inf]` as Threshold-based; everything else falls back to Manual with comma-joined finite edges). Flag is cleared at end of `_render_single_point` so the override is strictly one-shot — user edits after Load are preserved on subsequent renders. Global widget gains a `default_bin_edges` parameter (the Explorer widget already had one); `cadence.py:1087` threads `result['likelihood_bin_edges']` in at the call site when a loaded result is in session. The user explicitly rejected my first plan (a `_seed_bin_configs_from_loaded_result` helper that wrote to disk via `sm.save(...)` and popped all matching session-state keys) in favour of the much simpler one-shot session-flag pattern, noting that the widget code already has a `default_*` parameter path that can be gated by a flag. New learning recorded in `.claude/references/learnings.md` Context & Memory: "Prefer minimal session-state flags over helpers when the widget already has a `default_*` parameter path".

- **Bulk commit + push of accumulated validation pipeline work (commits `f40576e`, `faf92da`, `5bef671`, `a67f51c`, `d462fa5`; TODO #207 + #208).** Five logical commits pushed to main today during the `/git` workflow — the first big code push since 2026-04-19, covering ~2 weeks of uncommitted work that had been queued awaiting visual sign-off (per `feedback_no_self_approve`). Breakdown: (i) `.gitignore` update to exclude `results/` (2.3 GB), `mock_results/` (222 MB), `bin_sensitivity_results/`, `settings/states/`, `bias_app/.cache/` + `.obs_delta_rv_cache.npy`, `mock_inspector_app/__pycache__/`, `scripts/.agent_live_output.txt` + `.agent_work/`, and `papers/*.pdf`; (ii) **Model Explorer + validation pipeline** (23 files, +7432 / -820) — bundles Max ΔRV removal, truth attachment, Explorer bin editor + snapshots, smooth-CDF fix, `validation_io` module owning the `mock_results/` save schema, refresh of `helpers / params / cadence / runners_cadence / render_validation / render_shared / sim_plots / analysis / corner_plots / extras / file_ops / likelihood_viz / render_lk / render_lk_fit / render_shared_langer / sim_plots / validation / wr_bias_simulation`; (iii) **Agent skill restructure** — all `.claude/agents/*-skills/` subdirectories relocated to `.claude/skills/<role>/` to match the harness's updated skill discovery layout (pure renames, agent behaviour unchanged); (iv) **Verification tooling** — new `app/pages/13_tests_verification.py` + `_tests_verification_helpers.py` consolidating pipeline-correctness checks, standalone diagnostic scripts (`scripts/test_explorer_mock_equal{,_langer}.py`, `test_grid_vs_explorer_score.py`, `test_hdi_vs_nbins_frame.py`, `test_mock_distribution.py`, `make_powerlaw_explainer.py`), the standalone `mock_inspector_app/` Streamlit inspector for browsing generated mock observations on port 8503, `docs/sample_logP_powerlaw_explained.pdf`, and `environment.yml`; (v) **Refreshed plot PDFs** — regenerated `plots/agreement.pdf` + 6 siblings from the current pipeline state.

**Key results:**

- Mock Observation CDF on the Validation page now extends to the full data range (~900 km/s for typical truth params), not truncated at 460 km/s. Resolution control (bin width) preserved; the upper bound is computed from `np.ceil(max(obs) / bin_width) * bin_width + 5 * bin_width`.
- Corner-plot green dashed truth lines now appear on fresh validation runs (previously only after disk reload). All four truth axes (`fbin`, `pi`, `sigma`, `logPmax`) show their input value as a vline on the 1D diagonal and a green × marker on the 2D off-diagonals.
- Model Explorer logL now updates on every bin-edge change without recomputing the simulation (the CDF curve is built on `be`, the likelihood scoring uses `lk_be`; only the latter is varied by the editor). The cached `_run_grid_pipeline_via_cache` makes back-and-forth slider+bin motion instant after the first compute at each (params, n_sets) cell.
- Saved snapshot CDFs are byte-equivalent to the live Explorer line at the same params + n_sets (both call the same `smooth_pooled_cdf` and use the same lo/hi/median Plotly fill pattern).
- Per-page session-state size after 20 stacked mocks: tens of KB (only `delta_rv` + `is_binary` + 5 scalars stored per snapshot) — small enough that Streamlit reruns aren't perceptibly slowed.

**Methodology notes for paper:**

- **Auto-derived bin-grid upper bound.** The cadence-aware CDF bin grid no longer requires a user-facing `Max ΔRV` parameter; the upper bound is computed deterministically from `np.ceil(max(observed_ΔRV) / bin_width) * bin_width + 5 × bin_width`. This guarantees every observation falls within the grid (no overflow bin) and adds five headroom bins so the empirical step CDF reaches 1.0 visibly before the rightmost displayed bin. The simulation runner consumes the same edges, so display and likelihood scoring stay consistent.
- **Visual likelihood-bin sensitivity exploration.** The Model Explorer's tunable likelihood-bin editor exposes the bin choice as an interactive parameter while keeping the simulation parameters frozen. This lets the user visually diagnose whether a model's apparent poor fit is driven by the parameter values themselves or by the binning of the multinomial likelihood. The default Threshold-mode schema `[0, T, 250, 650, ∞]` (after Dsilva 2023, equation 4) places the detection-threshold edge at T and three coarse upper bins; the user can either sweep T (Threshold-based mode) or enter arbitrary edges (Manual mode). All snapshot logL values are computed against the same `obs_delta_rv` array, so they are directly comparable.
- **In-memory vs disk-state contract for validation runs.** Every background runner that persists state to disk via a sibling save-helper (with metadata passed as a separate argument) must mirror those metadata writes onto the in-memory `result` dict before returning. The on-disk format and the in-memory consumer must see the same shape. This is the generalisation underlying **E054** in `COMMON_ERRORS.md` and applies beyond the validation lane.

**Decisions:**

- **Removed Max ΔRV control entirely, did not gate behind an "auto/manual" toggle.** The user-facing knob's only purpose was to bound an internal bin grid; the deterministic derivation from `max(obs)` is always strictly better than a manually entered value (no truncation, no need to remember what number to type). Adding an auto/manual toggle would have re-introduced the same UX failure mode for users who left it on manual.
- **Snapshot store is session-only, no disk persistence.** The user explicitly stated that saved attempts are "only temporal so no need to save to disk". This matches the design intent: snapshots are for in-the-moment binning exploration, not a research artifact.
- **Snapshot dict captures `smooth_pooled_cdf` 5-tuple at Save time, not at draw time.** Storing the 5-tuple keeps Save instant (no recomputation when the snapshot is later redrawn) and makes the snapshot deterministic (the CDF shown later is exactly what was scored at Save time). Computing at draw time would have required the snapshot to remember enough state (full pooled_drv + seed + n_sets + lk_be + bin_cfg + period_model + cadence weights + ...) to be reproducible — fragile and slow.
- **Two separate settings namespaces** (`likelihood_bin_config` for the simulation, `explorer_likelihood_bin_config` for the Model Explorer) so the Explorer never overrides the simulation's saved bins. The user explicitly confirmed this constraint when approving the Explorer-bin-editor plan.
- **One-shot `is_loaded_result` session flag pattern, not a helper.** User-mandated: "you don't need a helper. the code already has a default value for the bins to pull from, create a global flag called 'is_loaded_result', so when its true, both fields starts with the bins from the results. it turns on when hitting the load button and starts as False." The simpler pattern has zero disk writes (the loaded run's bins do NOT overwrite the user's saved `likelihood_bin_config`), no helpers beyond a tiny `_infer_lk_bin_mode`, and gates an existing `default_bin_edges` parameter path that the widget already has. Strictly less code, strictly less coupling.
- **`results/` and `mock_results/` directories permanently gitignored.** 2.3 GB of `.npz` simulation outputs and 222 MB of mock-results were never meant to live in git history. The `.gitignore` update prevents future accidents and keeps the repo manageable for cloning.

**Bugs found and fixed:**

- **E054 (NEW)** — validation lane persists metadata to disk but not to the in-memory `result` dict. Fixed at `runners_cadence.py:672` by mirroring the disk metadata writes onto the in-memory `result` before `job['result'] = result`.
- Mock Observation CDF truncation at 460 km/s — fixed by removing the `Max ΔRV` user input and auto-deriving from observed ΔRVs (TODO #210).
- Snapshot CDF visual mismatch (binned step vs smooth + zero-fill shadow) — fixed by capturing the live `smooth_pooled_cdf` 5-tuple at Save time (TODO #212).

**Open questions:**

- **All five features status TO-TEST.** Visual sign-off needed across: (a) Max ΔRV removal + Mock Observation extending past 460 km/s, (b) green truth line on fresh validation runs, (c) Explorer bin editor + snapshot overlays + correct 16/84 bands on saved snapshots, (d) stacked mock CDFs (multiple clicks, dotted colored lines in cycled palette, Clear button works, recovery still uses most-recent), (e) bin-sync-on-Load for both Threshold and Manual saved results, including override-from-stale and user-edit-preserved regressions.
- **Pre-existing `test_render.py` failure at `render_lk_explorer.py:1810 'logL': float(_logL)` (NoneType)** flagged in three error-check runs today, unrelated to any of today's edits. Confirmed reproducible on baseline `d462fa5`. Worth investigating tomorrow — likely a test_render mock scaffold quirk (the harness mocks `_t_c2.button(...)` truthy, so the Save handler fires even when `_logL` is None) and not a real user bug.
- **Follow-on investigation queued by user:** "why when the simulation runs on the real parameters they don't get a better logL". Half of that (likelihood-bin variation) is now exposed via the Explorer editor; the other half (potential grading-method bug) is queued for tomorrow once visual sign-off is in.
- **User said "I'm done for today"** before some uncommitted code (today's flag/sync features) was committed — those are still in working-tree. EnDay will commit them tonight under a separate logical commit so the working version tag captures everything.

**New learnings (1):**

1. **Prefer minimal session-state flags over helpers when the widget already has a `default_*` parameter path.** Added to `.claude/references/learnings.md` Context & Memory. Triggered when the user rejected my first plan (helper writing to disk + popping keys) in favour of a 1-line flag pattern that gates an existing parameter path.
...
### 2026-05-20 — Validation sweep completed: 8 mock recoveries + real D'Silva inference + 3 real-anchored sub-runs; σ_single ↔ f_bin degeneracy isolated; Home page CDF panel; validation summary corner plot

**What was done:**

- **Validation experiment from TODO #217 executed and analysed.** The 8-mock sweep (Easy / Medium / Hard × upper-outlier / representative / bottom-outlier, with B1 rerun and B3 deferred) was run overnight and reviewed in the morning. Five structural patterns were extracted from the recovery table: (i) **f_bin is robust** — 7 of 8 mocks recovered the true binary fraction within 1σ across all difficulty regimes; (ii) **σ_single is biased low** under heavy-blend regimes — Case C mocks consistently pull the marginal max below the truth (σ=10), driven by a f_bin ↔ σ_single degeneracy where a higher binary fraction can absorb the same observed ΔRV spread that an honest σ_single inflation would otherwise explain; (iii) **π fails only on extreme tails** — the Hard regime upper-outlier sub-run is the only mock where π lands outside the 1σ HDI; (iv) **bottom-outlier asymmetry** — bottom-tail mocks fail differently from upper-tail mocks (σ collapses rather than expanding); (v) **B2 is the only fully-honest mock** — posterior contains all three truths and L_max/L_true ≈ 1.
- **Outlier-posterior honesty test (TODO #217 success/failure criterion).** The 2026-05-19 16:14 hypothesis stated that "if posteriors stay sharp under outlier mocks, the simulation reports a confident wrong answer when real data happens to be an outlier — no internal flag catches it". The 2026-05-20 results split the verdict by parameter: **f_bin passes the honesty test** (posteriors widen as expected and contain truth in 7/8 mocks); **σ_single fails the honesty test** in Case C (posterior stays sharp around the wrong value, no internal flag). New memory file `project_outlier_posterior_honesty_test.md` links the prediction to the result.
- **Real-data D'Silva inference completed and stored.** The 2026-05-20 19:15 D'Silva grid run on the X-Shooter sample (N=3000, sigma scan 1–16 with 50 steps, manual likelihood bin edges `0,12,20,45,150,350`) yielded **π = +2.455 +0.55/−0.97**, **σ_single = 4.37 +1.22/−0.80 km s⁻¹**, **f_bin = 0.646 +0.139/−0.162** (marginal max + 68% HDI per parameter via `compute_hdi68`). Saved as `results/cadence_dsilva_fb0.0-1.0x99_pi-3.0-3.0x100_N3000_sig1.0-16.0x50_260520-1915.npz`.
- **Real-data position falls in the previously-missing corner-plot quadrant.** Plotted on the σ-vs-π / σ-vs-f_bin / π-vs-f_bin corner plot, the real-data star lands at **high π ≈ 2.4, low σ_single ≈ 4.5, f_bin ≈ 0.65** — exactly the bottom-right quadrant identified earlier as a gap in the validation grid (the Case A/B/C mocks span π ∈ {−3, 0, +3} but only at low or medium σ). This means the data itself selected the diagnostic: 3 new real-anchored validation sub-runs (truth π=2.40, σ=4.5, f_bin=0.64; seeds 124/43/324 for upper/rep/bottom) were then launched.
- **Real-anchored sub-runs returned the best validation result of the whole sweep.** REAL.representative (seed 43) recovered π=+2.82 ±0.18/−1.21, σ=5.29 ±1.22/−1.43, f_bin=0.626 ±0.20/−0.12, with **L_max/L_true = 1.28** — the closest agreement between joint argmax and truth across all 12 mocks. REAL.upper (seed 124) and REAL.bottom (seed 324) showed the usual outlier asymmetries (REAL.upper at the π grid edge with f_bin = 0.263; REAL.bottom recovering σ but missing π). This is the first quantitative evidence that the real-data parameter region is itself well-recovered by the inference.
- **HDI / posterior-area rendering bug flagged.** Visual review identified that the σ_single marginal posterior shades the 68% HDI only to the LEFT of the marker — no error band above. Suspected cause: HDI truncated at grid edge or upper-tail integration dropping when the posterior is highly skewed. Separate ticket needed; not investigated this session.
- **Home page Observed ΔRV CDF panel (TODO #218) replaces K-S heatmap filler.** The K-S p-value heatmap block at `app/app.py:283-298` (~16 lines) was replaced with a ~115-line Observed ΔRV empirical CDF panel: black step CDF over the X-Shooter sample (stars where `is_binary is not None`), red single dots / green binary dots with horizontal `error_x` bars from `best_sigma`, orange dashed vline at the 45.5 km s⁻¹ detection threshold. Style constants imported from `bc.render_validation` (single source of truth for the validation-tab CDF look) per the inline comment at `render_validation.py:733`. A&A white-bg + Times-serif styling applied via `_AA_OVERRIDES`. The K-S p-value computation upstream (lines 102-117) is preserved because it still feeds the "Best K-S p" metric card and the Model Detection Fraction panel.
- **Validation summary plot script (TODO #219) — `scripts/plot_validation_summary.py` (~920 lines).** Two public pure functions: `make_validation_summary_figure(runs_df)` (single-panel σ-vs-π scatter) and `make_validation_corner_figure(runs_df)` (3-panel lower-triangle corner, with an L_max/L_true table in the unused (1,2) quadrant). `__main__` loads 9 mock `.npz` files plus 3 real-anchored sub-runs plus the real-data D'Silva inference, marginalises the 3D `likelihood` array over the other two axes per parameter, computes marginal max + 68% HDI via the shared `compute_hdi68`, and writes `plots/validation_summary_sigma_vs_pi.html` + `plots/validation_summary_corner.html`. Encoding pattern: **colour = case** (A blue, B green, C red, REAL purple), **shape = sub-run** (upper triangle / rep circle / bottom square / true plus / real star), **per-sub-run shade offset** (+0.55 upper / 0 rep / −0.30 bottom — asymmetric to balance "more noticeable" against "bottom too dark"), **rectangle-border dash pattern** (upper dash / rep dot / bottom solid / real longdashdot). Unicode box-drawing glyphs appended to legend labels (`┄ ┄ ┄`, `· · · · · · ·`, `──────`, `── · ── ·`) communicate the dash pattern to the user without occupying a separate legend group.

**Key results:**

- **D'Silva real-data inference (preliminary):** f_bin = 0.65 +0.14/−0.16, π = +2.46 +0.55/−0.97, σ_single = 4.37 +1.22/−0.80 km s⁻¹. **Not yet promoted to paper macros** — pending Langer-model repeat sweep [[pending_langer_validation_tests]] and resolution of the σ_single defensibility question (see below).
- **L_max / L_true diagnostic across 12 mocks:** REAL.rep = 1.28 (best agreement), C.bottom = 294, B.bottom = 46.7 (worst agreements). The wide spread at the bottom tail confirms the σ_single collapse mechanism. Within-file ratios cancel any per-file normalisation, so bin-edge differences across sub-runs do not break comparability.
- **Validation honesty verdict by parameter:** f_bin **passes** (7/8 within 1σ); σ_single **fails** in heavy-blend regimes; π **passes except on extreme tails**.

**Methodology notes for paper:**

- **f_bin ↔ σ_single degeneracy in the binned multinomial likelihood.** Both parameters affect the same observable — the high-ΔRV tail of the binary CDF — via different mechanisms. A higher f_bin adds more binaries (more high-ΔRV mass); a higher σ_single broadens the single-star wings (also more high-ΔRV mass, of a different shape). The current likelihood-bin layout (`0,12,20,45,150,350`) does not separate these contributions cleanly enough at the upper tail of the σ_single grid, so the joint posterior trades f_bin ↑ for σ_single ↓ along a degeneracy direction. This is the structural reason σ_single fails the outlier-honesty test in Case C and the real-data inference returns a low σ_single value alongside a high f_bin. Paper-direction options under consideration: (a) add an alternative scoring metric (Anderson–Darling or σ-coverage diagnostic) per [[pending_alt_scorer_model_explorer]] that breaks the degeneracy at the upper tail; (b) tighten the σ_single prior using external physical input (subtype-dependent wind variability floors from §4b.1); (c) report f_bin as the headline result and explicitly acknowledge the σ_single degeneracy without claiming a point estimate.
- **Validation experiment design — case × sub-run grid.** The 12-mock sweep (3 cases × 3 sub-runs + 3 real-anchored sub-runs) constitutes a 2D test in (case, outlier-position) where the case axis covers difficulty (Easy / Medium / Hard, plus the data-anchored fourth case at the inferred D'Silva position) and the sub-run axis covers within-realisation variance (upper-σ outlier / representative / bottom-σ outlier). Per [[honest_labels_always]] the recovered values are reported as **marginal max + 68% HDI per parameter**, never marginal max alone.
- **Inference at the real-data location is well-behaved.** The L_max/L_true = 1.28 for REAL.rep indicates the joint argmax sits within 0.25 in log-likelihood of the truth at the real-data parameter region. This is the strongest single-mock evidence that the inference at the D'Silva real-data position is honest about its uncertainty, in contrast to the σ_single failure at the Case-C location.

**Decisions:**

- **σ_single not promoted to paper macros yet.** The defensibility question (alt scorer, prior tightening, or honest acknowledgement) must be answered first; promoting `\sigmasingle` while the validation sweep flags it as biased would violate [[honest_labels_always]] and [[no_false_data]].
- **f_bin promotion deferred until Langer sweep completes.** Even though f_bin passed the honesty test for D'Silva, the bias correction must be defensible regardless of period-model choice. The Langer-model repeat sweep [[pending_langer_validation_tests]] is the gating dependency for `\fbincorr` / `\pibestfit` paper-macro updates.
- **Home page panel scope strictly limited to plot_col2.** The K-S p-value heatmap was replaced; the CDF Comparison panel in plot_col4 (also flagged as filler) was left untouched per the user's explicit scope.
- **Style constants imported from `bc.render_validation`, not duplicated.** The inline comment at `render_validation.py:733` establishes the validation-tab CDF style as the single source of truth; other modules must import rather than redefine. Applied to both the Home page panel and the validation-summary corner plot.
- **Asymmetric per-sub-run shading (+0.55 / −0.30) accepts contradictory feedback.** The user asked for "more noticeable" shading and then said "bottom too dark"; the asymmetric resolution (upper kept very light, bottom only moderately dark) honours both pulls instead of picking a single magnitude.
- **Real-data inference treated as a separate legend group, excluded from L_max/L_true table.** Real data has no truth, so the ratio is undefined; the table lists the 12 mocks only. The real-data star is rendered as a purple `★` in all four corner-plot panels and gets its own "Real data" legend group separate from "Mock cases" and "Sub-run".

**Bugs found and fixed:**

- **E018 dual-spread variant.** `fig.update_layout(**PLOTLY_THEME, **_AA_OVERRIDES)` raised `TypeError: got multiple values for keyword argument 'plot_bgcolor'` because both dicts contain `plot_bgcolor`, `paper_bgcolor`, `xaxis`, `yaxis`, `font` keys. Fix: merge into a single dict via `_merged = {**PLOTLY_THEME, **_AA_OVERRIDES}` first, then call `fig.update_layout(**_merged)`. Last-wins dict-merge resolves the duplicates. Extended COMMON_ERRORS.md E018 to cover the dual-spread variant with the additional colliding keys `plot_bgcolor`, `paper_bgcolor` and the dual-spread fix pattern.
- **No code defect from the perceived σ-rectangle bug.** User reported that REAL.upper's σ_single error rectangle appeared to include the real-data star in one corner-plot panel but not the other. Direct inspection of `fig.layout.shapes` confirmed the σ extent is byte-identical (5.286 to 6.790) in both panels — the star at σ=4.37 sits 0.92 km/s outside the rectangle in BOTH panels. Visual misreading driven by aspect-ratio asymmetry between the two panels (the rectangle's π-width is much larger than its σ-height in one panel and vice-versa). No code change.

**Open questions:**

- **σ_single defensibility decision.** Paper-direction options (alt scorer / tighter prior / honest acknowledgement) need a verdict before promoting the D'Silva real-data inference to paper macros.
- **B1 rerun (Medium upper-outlier) and B3 (Medium bottom-outlier).** First draws were not extreme enough for B1; B3 still pending. Both deferred to the next overnight slot.
- **Langer-model repeat sweep [[pending_langer_validation_tests]].** Required for paper-level claim that bias correction is robust to period-model choice.
...
- **Top-left corner of the σ-vs-π plane (high σ, low π)** remains un-tested. Lower priority now that the real-data position landed in the bottom-right (filling that quadrant naturally).
- **Visual sign-off pending** on (a) the Home page Observed ΔRV CDF panel (live `streamlit run app/app.py` needed); (b) the validation summary corner plot's Unicode dash-pattern legend glyphs (font-rendering sanity check across systems); (c) all prior `to-test` entries from #210–#216 plus the new #218/#219.
- **Pre-existing `render_lk_tab` TypeError in `scripts/test_render.py`** (`float()` on `NoneType`) confirmed unrelated to today's edits (reproduces on baseline). Not investigated today.

**New learnings (0):**

No new entries needed for `.claude/references/learnings.md`. The session reaffirmed two existing principles: (1) iterative styling = many small visual changes with quick `open html` rounds beats big design upfront; (2) when a user reports a perceived bug, extracting actual rendered data (`fig.layout.shapes` coordinates here) is faster than re-reasoning from source — and gives the user concrete numbers to verify against their eyes.
