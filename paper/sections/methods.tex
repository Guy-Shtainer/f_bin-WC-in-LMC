...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{plots/ccf_profile.pdf}
\caption{Worked example of the cross-correlation function for the
  \primaryline emission line on a representative epoch. The CCF profile
  (black) is shown together with the parabolic fit (red dashed) over the
  peak region above $f_\mathrm{fit} = \fitFracDefault$ of the maximum.
  The centroid of the parabola yields the radial velocity, and its
  $1\sigma$ uncertainty is derived from the curvature of the peak
  following \citet{Zucker2003}.}
\label{fig:ccf_profile}
\end{figure}
...
\hline
\end{tabular}
\tablefoot{$\lambda_\mathrm{min}$ and $\lambda_\mathrm{max}$ delimit the
default integration window adopted for the cross-correlation of each line;
they are not the rest-frame line centres, which appear in the Line ID
column. Per-star, per-line overrides (Sect.~\ref{sec:ccf}) are released
with the public CCF settings file.}
\end{table}
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{plots/agreement.pdf}
\caption{Line-agreement cross-check across the \NlinesAll candidate
  emission lines. \textit{Left:} per-line agreement score $S_\ell$
  (Eq.~\ref{eq:agreement_score}) ranked from highest to lowest, computed
  from the Pearson correlation of per-star $\Delta\mathrm{RV}_{\max}$
  values across line pairs, weighted by sample overlap with a minimum of
  \Nstarsmincorr common stars. \textit{Right:} agreement score versus
  mean per-epoch equivalent width of the line. The positive correlation
  supports the depth-of-formation argument: the strongest, deepest-formed
  lines are also the most kinematically self-consistent across the
  sample. The \primaryline doublet emerges as both the strongest and the
  most agreeable line.}
\label{fig:agreement}
\end{figure*}
...
\begin{figure}
\centering
% \includegraphics[width=\columnwidth]{plots/threshold_derivation.pdf}
\caption{Determination of the binary detection threshold from the
  cumulative observed binary fraction. The step curve shows the fraction
  of stars with $\Delta\mathrm{RV}_{\max} > T$ as a function of threshold
  $T$; the dashed curves show the two Gaussian components of the
  parametric mixture model (Eq.~\ref{eq:gauss_threshold}); the solid red
  curve is the joint best fit. The vertical line marks the adopted
  threshold $T = \drvthresh$\,\kms.}
\label{fig:threshold_derivation}
\end{figure}
