...
def compute_model_fraction_curve(
    ...
    sigma_measure_single: float = 0.0,
    error_model_binary: str = 'fixed',
    error_params_binary: tuple = (),
    sigma_measure_binary: float = 0.0,
) -> tuple[np.ndarray, np.ndarray]:
    """Parametric model with per-epoch noise and 2-test detection.

    Singles: one intrinsic draw per star tiled across epochs.
    Binaries: per-epoch intrinsic draws.
    Per-epoch measurement noise added independently.
    Detection: (dRV > t) AND (dRV - 4*sigma_pair > 0), sigma_pair = sqrt(2)*sigma_measure.
    """
    from wr_bias_simulation import _draw_measurement_noise
    ...
    v = np.zeros((n_sim, n_epochs), dtype=float)

    if n_sin > 0:
        v_sin_intrinsic = scipy_single.rvs(*params_single, size=(n_sin, 1),
                                           random_state=rng)
        v_sin = np.broadcast_to(v_sin_intrinsic, (n_sin, n_epochs)).copy()
        noise_sin = _draw_measurement_noise(
            error_model_single, error_params_single,
            sigma_measure_single, size=(n_sin, n_epochs), rng=rng)
        v_sin += noise_sin
        v[~is_binary] = v_sin

    if n_bin > 0:
        v_bin = scipy_binary.rvs(*params_binary, size=(n_bin, n_epochs),
                                 random_state=rng)
        noise_bin = _draw_measurement_noise(
            error_model_binary, error_params_binary,
            sigma_measure_binary, size=(n_bin, n_epochs), rng=rng)
        v_bin += noise_bin
        v[is_binary] = v_bin

    delta_rv = v.max(axis=1) - v.min(axis=1)

    sigma_pair = np.empty(n_sim, dtype=float)
    sigma_pair[is_binary] = np.sqrt(2.0) * float(sigma_measure_binary)
    sigma_pair[~is_binary] = np.sqrt(2.0) * float(sigma_measure_single)

    t_arr = np.arange(0, t_max, dtype=float)
    sig_term = delta_rv - 4.0 * sigma_pair
    f_curve = np.array([
        float(np.mean((delta_rv > t) & (sig_term > 0.0)))
        for t in t_arr
    ])
