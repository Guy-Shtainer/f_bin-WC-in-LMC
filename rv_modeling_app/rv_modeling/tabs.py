...
def render_tab_sample_fit(obs_data: dict):
    ...
    if should_run:
        with st.spinner("Computing ΔRVs and fitting models..."):
            ...
            binary_drvs = compute_binary_delta_rvs(
                n_sim=p.get("n_sim", 10000),
                n_epochs=p.get("n_epochs", 8),
                time_span=p.get("time_span", 365.0),
                period_model=p.get("period_model", "powerlaw"),
                pi=p.get("pi", 0.0),
                e_model=p.get("e_model", "flat"),
                e_max=p.get("e_max", 0.9),
                q_model=p.get("q_model", "flat"),
                seed=p.get("seed", 42),
                weight_A=p.get("weight_A", 0.3),
                logP_min=p.get("logP_min", 0.15),
                logP_max=p.get("logP_max", 5.0),
                q_min=p.get("q_min", 0.1),
                q_max=p.get("q_max", 2.0),
                q_flipped=p.get("q_flipped", False),
                langer_q_mu=p.get("langer_q_mu", 0.65),
                langer_q_sigma=p.get("langer_q_sigma", 0.3),
                mass_primary_model=p.get("mass_model", "fixed"),
                mass_primary_fixed=p.get("mass_fixed", 10.0),
                mass_primary_min=p.get("mass_min", 10.0),
                mass_primary_max=p.get("mass_max", 20.0),
                dist_A=p.get("dist_A", "gaussian"),
                mu_A=p.get("mu_A", 0.8),
                sigma_A=p.get("sigma_A", 0.35),
                dist_B=p.get("dist_B", "reflected_lognormal"),
                mu_B=p.get("mu_B", 2.0),
                sigma_B=p.get("sigma_B", 0.45),
            )
            sorted_binary = np.sort(binary_drvs)
            sorted_std_ranges = compute_standard_ranges(p.get("n_epochs", 8))
    ...
    fig1.update_yaxes(title_text="Binary fraction", title_font=_tf, row=1, col=1, **{
        k: v for k, v in _ay.items() if k != "title"})
    fig1.update_yaxes(title_text="(Obs−Model)/σ", title_font=_tf, row=2, col=1, **{
        k: v for k, v in _ay.items() if k != "title"})
    fig1.update_xaxes(title_text="ΔRV threshold (km/s)", title_font=_tf, row=2, col=1, **{
    ...
    fig2.update_layout(**{
        **PLOTLY_THEME,
        ...
        "xaxis": {**_ax, "title": {**_ax.get("title", {}), "text": "ΔRV (km/s)"}},
        "yaxis": {**_ay, "title": {**_ay.get("title", {}), "text": "Weighted PDF"}},
        "legend": {**_al, "x": 0.98, "y": 0.98, "xanchor": "right"},
    })
    ...
    fig_h.update_layout(**{
        **PLOTLY_THEME,
        ...
        "xaxis": {**_ax, "title": {**_ax.get("title", {}), "text": "ΔRV (km/s)"}},
        "yaxis": {**_ay, "title": {**_ay.get("title", {}), "text": "Count"}},
        "legend": {**_al, "x": 0.98, "y": 0.98, "xanchor": "right"},
        "barmode": "overlay",
    })
    ...
    for mdl, label, clr, dash in [
        ...
        fig_r.update_layout(**{
            **PLOTLY_THEME,
            ...
            "xaxis": {**_ax, "title": {**_ax.get("title", {}), "text": "ΔRV threshold (km/s)"}, "range": [0, 250]},
            "yaxis": {**_ay, "title": {**_ay.get("title", {}), "text": "Recovered f_bin"}, "range": [-0.1, 1.1]},
...
def render_tab_global_correction(obs_data: dict):
    ...
    for mdl, label, clr in [
        ...
        fig_g.update_layout(**{
            **PLOTLY_THEME,
            ...
            "xaxis": {**_ax, "title": {**_ax.get("title", {}), "text": "ΔRV threshold (km/s)"}},
            "yaxis": {**_ay, "title": {**_ay.get("title", {}), "text": "Binary fraction"}},
            "legend": {**_al, "x": 0.98, "y": 0.98, "xanchor": "right"},
            "height": 450,
        })
    ...
    _tf = dict(family='Times New Roman, serif', size=13, color='black')
    for c in [1, 2]:
        fig_rv.update_xaxes(title_text="Centered RV (km/s)", title_font=_tf, row=1, col=c, **{
            k: v for k, v in _ax.items() if k != "title"})
        fig_rv.update_yaxes(title_text="Count", title_font=_tf, row=1, col=c, **{
            k: v for k, v in _ay.items() if k != "title"})
    st.plotly_chart(fig_rv, use_container_width=True, theme=None)
