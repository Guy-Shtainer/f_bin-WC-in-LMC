...
from shared_lite import PLOTLY_THEME
...
_RVE_PARAM_META: dict[str, list] = {
    ...
    'Laplace': [('μ (loc)', 0.0, -50.0, 50.0, 0.01),
                ('b (scale)', 1.0, 0.01, 50.0, 0.01)],
    'Gen. Normal': [('β (shape)', 1.0, 0.1, 10.0, 0.01),
                    ('loc', 0.0, -50.0, 50.0, 0.01),
                    ('scale', 1.0, 0.01, 50.0, 0.01)],
}
...
def render_orbital_histograms(
    ...
    for pi in range(9):
        ...
        fig.update_xaxes(title_text=_X_LABELS[pi], showgrid=False, row=r, col=c,
                         title_font=dict(family='Times New Roman, serif', size=13, color='black'))
    ...
    st.plotly_chart(fig, use_container_width=True, key=f'{key_prefix}_orb_hist', theme=None)
