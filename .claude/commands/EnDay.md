...
2. **Audit TODO.md:**
   - Read `TODO.md` and the git log from Step 1.
   ...
   - **Scan conversation context for missing tasks:** Look for work the user mentioned, assigned, or discussed during the session that has no corresponding TODO.md entry. Add new entries with proper attribution, full datetime, and status (`to-test` if completed, `open` if not yet started).
   - **Report changes:** List every TODO.md update (status changes and new entries) so the user can review before proceeding.

3. **Document errors in COMMON_ERRORS.md:**
...
4. **Update DOCUMENTATION.md Section 7 (Work Log):**
...
5. **Update the paper (paper/sections/*.tex):**
...
6. **Commit changes (one logical change per commit):**
