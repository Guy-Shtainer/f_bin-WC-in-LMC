...
| Hash | Summary |
|------|---------|
| `8c421d2` | End-of-day 2026-05-10 docs: TODO entries 204-206, DOCUMENTATION §7, daily log, COMMON_ERRORS E048 recurrence, learnings |

**Major changes (committed):**
- **Docs end-of-day (2026-05-10):** TODO.md entries #204 (Validation Model Explorer CDF — smooth pooled empirical + 16/84 percentile band, applied to both Dsilva `render_lk_explorer.py:1124` and Langer `render_lk_explorer_langer.py:698`, reusing the existing `bc.helpers.smooth_pooled_cdf` helper; band convention 16/84 percentile matches project standard; original step trace kept as the empty-pool fallback), #205 (standalone `mock_inspector_app/` Streamlit webapp on port 8503 — 5 files ~2200 lines comparing Mock Data vs Model Explorer pipelines side-by-side at the same params/seed; 3×3 orbital histograms grid + ΔRV CDF + f_bin-vs-threshold curve per pipeline + multi-seed single-run accumulation; 8 iteration rounds; cross-tab divergence diagnosed as σ_meas default mismatch — 5.0 in new GUI vs 1.622 in validation tab; `mp.Pool(spawn)` parallelisation reverted because it blocks the Streamlit thread for 5–10 s on macOS), and #206 (Validation mock-vs-grid CDF mismatch fix at `render_validation.py:347-404` — UI mock generator built fresh `BinaryParameterConfig` with only 5 fields, leaving `q_range` at default `(0.1, 2.0)` while the grid worker used `(0.1, 4.0)` from the orbital-params widget; same root cause as **E048** at a new call site; fix builds `_base_bin_cfg` from `settings['grid_cadence_*']['orbital']` then `dataclasses.replace(...)` for truth-specific overrides). DOCUMENTATION.md §7 gains a 2026-05-10 entry with key results (q_range CDF offset 0.05–0.09 numerically confirmed; cross-tab σ_meas defaults audit; 5 different binary-index sampling methods across the codebase), methodology notes for the paper (pooled empirical CDF generalised to Model Explorer; result-dict completeness for re-sim helpers extended to UI mock generators; σ_meas affects ΔRV directly not just an errorbar), 3 new learnings (don't dismiss user-reported divergence after one incomplete bisect; σ_meas affects ΔRV directly; run empirical comparison BEFORE re-presenting "single-mock noise" arguments). COMMON_ERRORS.md **E048** extended with 2026-05-10 recurrence note (same root cause at a new call site, no new E-number minted). `.claude/references/learnings.md` last_triggered → 2026-05-10 on the two existing rules about full bin_cfg / period_model threading; new entry under Data & Debugging on running empirical comparison BEFORE repeating "single-mock noise" arguments. Daily log gained 3 conversation entries (15:56, 17:42, 17:45).

**Uncommitted on `main` (today's code work + 7-day backlog from `v260507-working` and earlier, all pending visual sign-off):**
- **TODAY — Validation Model Explorer CDF smoothing (TODO #204, 2 files).** `app/bc/render_lk_explorer.py` and `app/bc/render_lk_explorer_langer.py` — three hunks per file (import + Explorer trace block + best-fit overlay block). Reused the LOCAL `_hex_to_rgba` already defined at the top of each file instead of re-importing from `bc.helpers` (avoids shadowing/redefinition warning). Current = dashed red @ 0.20 fill opacity; best-fit = dotted red @ 0.12 fill opacity (so the two bands compose without illegibility). Dropped `shape='hv'` on the median line.
- **TODAY — Standalone `mock_inspector_app/` (TODO #205, 5 new files + auto-created settings.json).** `mock_inspector_app/__init__.py`, `mock_inspector_app/settings.py`, `mock_inspector_app/runner.py`, `mock_inspector_app/inspector.py`, `mock_inspector_app/app.py` (~2200 lines). Imports from existing project (`bc.helpers`, `wr_bias_simulation`, `pipeline.load_observations`) but writes to its own `mock_inspector_app/settings.json`. No changes to existing project files (`app/**`, `wr_bias_simulation.py`, `pipeline/**`, `bc/**` all untouched per "don't touch working code" rule). Launch: `conda run -n guyenv streamlit run mock_inspector_app/app.py --server.port 8503`.
- **TODAY — Validation q_range fix (TODO #206, 1 file).** `app/bc/render_validation.py:347-404` — replaced 11-line `BinaryParameterConfig(...)` construction with `_base_bin_cfg` from settings + `dataclasses.replace(...)` overrides. Both Dsilva and Langer branches handled. Override only truth-specific fields (`logP_min`, `logP_max`, `period_model`, `e_model`, `e_max`); everything else (q_range, mass_primary_*, langer_*, q_flipped) inherits from the user's orbital-params widget.
- **Rolling backlog (TODO #200, #198, #199, #196, #192, #191, #190, #185, #202, #203, …)** — 14 `app/bc/` files, `wr_bias_simulation.py`, `app/shared.py`, several `Backups/`, plus today's three sessions. All unchanged from prior days. See 2026-05-07 / 2026-05-04 / 2026-04-30 entries for full file lists.

**Pending visual verification (next session — multi-sprint hand-off, growing list):**
- **(today)** Open Bias Correction → Validation tab → expand Model Explorer (Dsilva and Langer pages independently) → confirm smooth dashed red line + translucent red band on Explorer (current); dotted red + lighter band on best-fit overlay when checkbox ticked; both bands compose readably; band tracks slider movement; `vrect` bin overlay still visible behind new traces.
- **(today)** q_range fix — set truth + `q_range=(0.1, 4.0)`, Generate, verify the mock empirical CDF max ΔRV is now in typical range for q≤4 (often >200; fresh code at seed=42 gives max=244). Validation grid run should land Explorer-at-truth logL within single-mock noise of the algorithm's best.
- **(today)** Optional: launch `mock_inspector_app/` on port 8503 and exercise the Mock vs Explorer side-by-side comparison + multi-seed single-run accumulation + 16/84 std band on f_bin panel.
- **(rolling forward)** All 2026-05-07 / 2026-05-04 / 2026-04-30 / 2026-04-29 / 2026-04-28 / 2026-04-23 visual checks listed in the prior GIT_LOG entries — still pending.

**Pending decisions (next session):**
- σ_meas default in `mock_inspector_app/` — change 5.0 → 1.622 to match validation tab? Two options: (a) change default + leave knob; (b) keep at 5.0 with caption noting validation-tab default.
- Bernoulli f_bin sites in `wr_bias_simulation.py:1073, :2132` — violate the deterministic-mock rule per `feedback_mock_deterministic.md`. Audit and standardise.
- Other call sites of `BinaryParameterConfig(...)` with subset-of-fields construction — broader audit grep `BinaryParameterConfig\(` and triage each. E048 grep is too narrow (only matches `logP_max=` first-arg form).

---

## 2026-05-07 — End-of-day docs only: corner-plot truth markers + smooth pooled-empirical CDFs + π explainer PDF + mock-data verification UNCOMMITTED awaiting visual sign-off

**Tag:** `v260507-working` *(docs only — code work uncommitted; the four sessions today touched 14 `app/bc/` files and 3 new scripts/pages, all carrying TODO `to-test` status pending visual confirmation. The `v260430-*` and 2026-05-04 backlog remains on `main` unchanged.)*

| Hash | Summary |
|------|---------|
| `be84aa4` | End-of-day 2026-05-07 docs: TODO entries 200-203, DOCUMENTATION §7, daily log, COMMON_ERRORS E052/E053, learnings |

**Major changes (committed):**
- **Docs end-of-day (2026-05-07):** TODO.md entries #200 (validation corner plot — green dashed "true input" markers on diagonals + 2D heatmaps; 3-file patch covering `corner_plots.py` + `render_lk_fit*.py` after QA agent caught the duplicate-helper bypass), #201 (π parameter explainer PDF — `scripts/make_powerlaw_explainer.py` → `docs/sample_logP_powerlaw_explained.pdf`, 6 pages tracing slider → `sample_logP_powerlaw` inverse-CDF), #202 (smooth pooled-empirical CDFs across all 5 simulated-star plot sites — new `bc.helpers.smooth_pooled_cdf` helper applied to `render_shared{,_langer}.py`, `render_lk_fit{,_langer}.py`, `likelihood_viz.py`), and #203 (mock-data distribution verification — `scripts/test_mock_distribution.py` 5/5 PASS, parallel-Claude's "wrong distribution" claim REFUTED; built `app/pages/13_tests_verification.py` generic test-runner page that turned out to be the wrong shape for the user's "new GUI for verifying mock data" ask). DOCUMENTATION.md §7 gains a 2026-05-07 entry with methodology notes for the paper (pooled empirical CDF as the single visual primitive for simulated-star CDFs, π=−1 prior is uniform in `ln(log P)` not in P, two-source observational model with deterministic `n_bin = round(N × f_bin)`). COMMON_ERRORS.md gains **E052** (`\bigl`/`\bigr` family unsupported by matplotlib mathtext, crashes at draw time) and **E053** (`ax.text(..., wrap=True)` corrupts `$math$` segments — the wrapper inserts newlines mid-formula); Quick-Scan Regex updated. `.claude/references/learnings.md` Plot Rendering section gains "always rasterise + visually inspect every page of multi-page PDFs before declaring done" — the explainer build hit three separate visual-only regressions (mathtext crash, page-1 formula raw-LaTeX render, page-6 caption math literals). Daily log gained 4 conversation entries (14:38, 17:34, 17:43, 17:46). New memory file `feedback_new_gui_means_dedicated_interactive.md` linked from MEMORY.md Critical Rules — when user asks "build a new GUI for X", they mean a dedicated interactive inspector with X-specific controls and live plots, NOT a generic CLI wrapper / test-runner page.

**Uncommitted on `main` (today's code work + 7-day backlog from `v260430-*` and 2026-05-04, all pending visual sign-off):**
- **TODAY — Validation corner plot truth markers (TODO #200, 3 files).** `app/bc/corner_plots.py` (`_render_corner_plot`, used by K-S / CvM / weighted-CvM tabs); `app/bc/render_lk_fit.py` and `app/bc/render_lk_fit_langer.py` (`_render_lk_corner_plot`, the duplicate Likelihood-tab implementation that caused round-1 to silently no-op on the user's screenshot). Same 4-edit pattern in each: `_add_1d_posterior` gains `true_value=None` + green `add_vline`; `_add_2d_heatmap` gains `true_x=None, true_y=None` + green × Scatter; `_render_*_corner_plot` gains `_truth_keys` + `_is_validation` + `_truth_for(axis_name)` gated on `result.get('is_validation', False)`; caption gains conditional truth-marker clause. Updated WORKING-flagged comment banners in `render_lk_fit*.py` to `UPDATED 2026-05-06: added validation truth-line markers — user authorised`.
- **TODAY — π explainer PDF (TODO #201, 1 script + 1 PDF).** `scripts/make_powerlaw_explainer.py` (new) imports the production `sample_logP_powerlaw` and emits `docs/sample_logP_powerlaw_explained.pdf` (new dir) via `matplotlib.backends.backend_pdf.PdfPages`. 6 pages: title + summary, derivation of `(π+1)` exponent, prior PDFs / CDFs / histograms / period-in-days view across π ∈ {−2, −1, −0.5, 0, 1, 2}.
- **TODAY — Smooth pooled-empirical CDFs (TODO #202, 6 files).** `app/bc/helpers.py` gains `smooth_pooled_cdf(pooled, n_sets, n_fine=500)` returning `(sorted_pool, y_pool, x_fine, lo_fine, hi_fine)`. Applied verbatim to `app/bc/render_shared.py`, `app/bc/render_shared_langer.py`, `app/bc/render_lk_fit.py`, `app/bc/render_lk_fit_langer.py`, `app/bc/likelihood_viz.py`. Brought `render_shared_langer.py` to parity with the powerlaw twin; the Likelihood Best-Fit observed CDF is now a true empirical step (`np.sort(obs_drv)`, `shape='hv'`). `memory/plot_preferences.md` updated with split rule (observed `shape='hv'`, simulated pooled empirical + smooth band).
- **TODAY — Mock-data verification + Tests page (TODO #203, 3 new files + 1 script).** `scripts/test_mock_distribution.py` (5-case verification suite — REFUTES parallel-Claude's claim); `app/pages/13_tests_verification.py` + `app/pages/_tests_verification_helpers.py` (generic test-runner page — wrong shape for user's actual ask, but functionally clean and kept on disk per "we will not do that now"). New memory `feedback_new_gui_means_dedicated_interactive.md`.
- **Rolling backlog (TODO #200, #198, #199, #196, #192, #191, #190, #185, …)** — 14 `app/bc/` files, `wr_bias_simulation.py`, `app/shared.py`, several `Backups/`, all unchanged from prior days. See 2026-05-04 / 2026-04-30 entries for full file lists.

**Pending visual verification (next session — multi-sprint hand-off, growing list):**
- **(today)** Open Bias Correction → Validation tab → Likelihood scoring tab → expand corner plot → confirm green dashed vlines on each diagonal panel at the chosen truth values + green × on each 2D heatmap. Also K-S / CvM / Weighted tabs (shared `corner_plots.py` path).
- **(today)** 5 smooth-pooled-CDF panels (Dsilva + Langer twins for mock-vs-best-fit and Likelihood Best-Fit, plus alt panel in `likelihood_viz.py`). Pending follow-ups: Model Explorer CDF in `render_lk_explorer.py` still coarse; "last CDF of Panel B" also still coarse.
- **(rolling forward)** All 2026-05-04 / 2026-04-30 / 2026-04-29 / 2026-04-28 / 2026-04-23 visual checks listed in the prior GIT_LOG entries — still pending.

**Next-session feature work:**
- Build the actual mock-data inspector GUI (sliders for σ_single, σ_meas, f_bin, error_model, true_logPmax → live `bc.validation.generate_mock_observations_detail` → per-epoch RV histogram vs theoretical N(0, √(σ_single² + σ_meas²)), is_binary count vs round(N·f_bin), ΔRV CDF, KS verdict overlay). The generic Tests page (`app/pages/13_tests_verification.py`) is left on disk but does NOT solve this ask.
