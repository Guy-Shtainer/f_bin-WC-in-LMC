"""Byte-identical CDF assertion: Mock vs Explorer-validation-mode (LANGER).

Langer twin of scripts/test_explorer_mock_equal.py.  When the Langer Explorer's
_me_cdf_band_langer is called with validation_mode=True and the SAME seed used
to generate the mock, the returned ΔRV array must equal the mock's delta_rv
byte-for-byte (same RNG draws → same values).

Usage:
    conda run -n guyenv python scripts/test_explorer_mock_equal_langer.py
"""
...
# Mirrors the pattern in scripts/test_explorer_mock_equal.py.
...
from bc.render_lk_explorer_langer import _me_cdf_band_langer  # noqa: E402
...
def _run_case(label: str, error_model: str, error_params: tuple) -> int:
    ...
    # Test parameters — Langer period model on both sides.
    fbin = 0.46
    pi = 0.0            # ignored by langer2020 branch of sample_logP
    sigma = 15.0
    logPmax = 4.0
    seed = 42
    ...
    # Langer mock uses e_model='zero' (matches render_validation.py:254).
    bin_cfg = BinaryParameterConfig(
        logP_min=0.15, logP_max=logPmax,
        period_model=period_model, e_model='zero', e_max=0.9,
    )
    ...
    # ── (3) Langer Explorer validation-mode path ─────────────────────────
    # _me_cdf_band_langer returns (med, lo, hi, pooled).  validation_mode
    # forces pooled to be a single draw equal to the mock's delta_rv.
    med, lo, hi, pooled = _me_cdf_band_langer(
        fbin, logPmax, sigma, sigma_meas, tuple(be.tolist()),
        n_sets=1,
    ...
    print(f'  PASS — Langer {label}: byte-identical invariant holds.')
    return 0
...
def _assert_sigma_meas_actually_perturbs_cdf() -> int:
    ...
    """
    print('--- Regression guard (Langer): σ_meas must affect ΔRV ---')
    fbin = 0.46
    pi = 0.0  # ignored by langer2020
    sigma = 15.0
    logPmax = 4.0
    seed = 42
    period_model = 'langer2020'
    ...
    print('  PASS — σ_meas demonstrably perturbs the Langer mock CDF '
