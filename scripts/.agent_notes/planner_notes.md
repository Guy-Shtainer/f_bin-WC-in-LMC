...
- **Plan interactive playground sections carefully — specify what is re-computed vs cached** — the Parameter Playground reuses cached survival function arrays from session_state (no re-simulation), only recombining the mixture weights. Plans should explicitly state which data persists between interactions and which is recomputed, to avoid unnecessary re-simulation or stale data bugs.
- **When planning new pages, include integration steps for `shared.py` navigation and `app.py` dashboard** — every new page requires: (1) navigation entry in `shared.py` (position matters), (2) workflow checklist item in `app.py`. Forgetting either makes the page unreachable or invisible in the dashboard overview.
- **Plan preset button UX as a separate step** — preset buttons (Dsilva/Langer) that change multiple sidebar parameters need explicit planning: which session_state keys to set, what default values, and whether `st.rerun()` is needed. This avoids partial-update bugs where some widgets reflect old values.
- **Specify session_state serialization constraints in plans** — `interp1d`, lambda functions, and other non-picklable objects cannot be stored in `st.session_state`. The plan should specify storing raw arrays and reconstructing callable objects where needed.
