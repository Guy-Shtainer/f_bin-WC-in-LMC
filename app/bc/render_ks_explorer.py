"""bc.render_ks_explorer — K-S interactive exploration tools.
...
Hardcoded for K-S scoring — no likelihood/CvM branches.
"""
...
from bc.helpers import SCORING_METHODS, _METHOD_COLORS, _hex_to_rgba
...
def _render_ks_resim_interp(interp, result, x_label, pfx):
    """Re-simulate CDF at interpolated best-fit point for K-S scoring."""
    st.markdown('#### Re-simulate at Interpolated Point')
    c1, c2, c3 = st.columns([0.3, 0.3, 0.4])
    ns = c1.number_input('N_sets', 100, 50000, 1000, step=100, key=f'{pfx}_ks_resim_n')
    if not c2.button('Re-simulate', key=f'{pfx}_ks_resim_btn', type='primary'):
        return
    try:
        ...
        st.plotly_chart(fig, use_container_width=True, key=f'{pfx}_ks_resim_cdf')
        c3.metric('K-S p-value (interp)', f"{interp.get('S', 0):.6f}")
...
def _render_ks_cdf_sanity_check(best_fbin, best_x, sigma_single,
    ...
    pipeline produces sensible DeltaRV distributions.
    """
    from wr_bias_simulation import (
    ...
    fig.update_layout(**{
        **PLOTLY_THEME,
        'title': dict(
            text=f'CDF Sanity Check  (f_bin={best_fbin:.3f}, 25 stars x 5 draws)',
            font=dict(size=14)),
...
def _render_ks_model_explorer(
    ...
    if _lp_g.size > 1:
        ...
        me_logPmax = _c.slider(
            ...
            0.1, key=f'{prefix}_ks_me_logPmax')
    ...
    # ── Score metric (K-S only) ─────────────────────────────────────────
    _D, _pv = ks_two_sample_binned(pooled_drv, obs_drv, be)
    _score_val = f'{_pv:.6f}'

    sc1, sc2 = st.columns([0.35, 0.65])
    sc1.metric('K-S p-value', _score_val)
    sc2.caption(f'f_bin={me_fb:.3f}, {x_label}={me_x:.2f}, sigma_single={me_sig:.1f} km/s')
    ...
    st.plotly_chart(fig_det, use_container_width=True,
                    key=f'{prefix}_ks_me_det')

    st.caption(
        f'K-S model explorer for {display_name}. '
        f'CDF shows median +/- 68% band from 50 simulations. '
        f'K-S p-value computed from pooled simulated data.'
    )
...
def render_ks_explorer(
    ...
    method_key : str
        Scoring method key (always 'ks' for this module).
    display_name : str
        Display name for the method.
    best_fb, best_x, best_sig : float or None
        Best-fit values to use as slider defaults. If None, derived from result.
    ...
    # Get K-S p-value array for info computation
    ks_p = result.get('ks_p')
    if ks_p is not None:
        p_nd = np.asarray(ks_p, dtype=float)
    else:
        st.info('No K-S p-value data available.')
        return
    ...
    info = _method_best_and_hdi(p_nd, _grids, _names, is_likelihood=False)

    # Override info best values if caller provided explicit ones
    if info is not None and (best_fb is not None or best_x is not None or best_sig is not None):
    ...
    if _interp is not None:
        _render_ks_resim_interp(_interp, result, x_label, p)
    ...
    if _cadence_lib is not None and obs_delta_rv is not None:
        ...
        _pm = 'dsilva'  # default; caller should set based on tab type
        # Detect period model from result metadata
        if result.get('period_model') == 'langer':
            _pm = 'langer'
        try:
            _render_ks_cdf_sanity_check(
                _bv.get('fbin', 0.5), _bv.get(x_name, 0.0),
                _bv.get('sigma', float(result.get('sigma_meas', 5.0))),
                np.asarray(obs_delta_rv), _pm, result, f'{p}_{method_key}')
        except Exception:
            pass
