...
def _render_cvm_1d_plot(col, t_grid, S_grid, label, best_t, best_S,
    ...
    # (D9 Parabola — Dsilva).  See task brief — interpolation section A&A pass.
    try:
...
def _render_lk_corner_plot(p_nd, fbin_g, x_g, x_name, x_display_label,
    ...
    with st.expander(f'Corner Plot -- {_DISPLAY_NAME}', expanded=False):
        ...
        if ndim_mode in ('dsilva', 'cadence_dsilva'):
            _all_grids = []
            _all_names = []
            if _has_lp:
        ...
        elif ndim_mode in ('langer', 'cadence_langer'):
            _all_grids = []
            _all_names = []
            if _has_lp:
                _all_grids.append(_logPmax_g)
        ...
        # User authorised 2026-04-28: A&A theme override applied inside WORKING block
        # (D14 Corner Plot — Dsilva).  See task brief — corner plots A&A pass.
        try:
            from bc.render_validation import _AA_OVERRIDES
...
def _render_likelihood_cdf(
    ...
    # Observed CDF: classic empirical step from observed ΔRVs.
    ...
    # Simulated CDF: smooth empirical CDF of pooled ΔRVs + 500-point band.
