...
| F-045 | **Grid range exclusion UI**: range sliders for ALL axes (f_bin, π, σ_single, logP_max). N-D boolean mask matches likelihood array shape. Excluded points = NaN on heatmaps (white). Best-fit star + downstream scoring update. **WORKING ✓ (2026-04-09)** | `helpers.py:99-197` + `cadence.py` | Narrow any slider → excluded region turns white; best-fit star moves; caption shows count |
| F-046 | **Exclusion mask persistence**: N-D mask applied at cadence.py level to heatmaps + model_ctx. 2D projection stored in `{prefix}_exc_mask_2d` for backward compat with scoring_detail consumers. **WORKING ✓ (2026-04-09)** | `helpers.py` + `cadence.py` | Set exclusion → navigate away → return → exclusion still applied |
...
| F-052 | **Cancel & Save button** ("💾 Cancel & Save"): appears alongside Cancel. Sets `cancel_mode = 'save'` — background thread writes partial checkpoint .npz before exiting. Partial file includes `progress_pct`, `rows_done`, `total_rows` alongside scoring arrays (NaN for incomplete cells). **WORKING ✓ (2026-03-29)** — fixed scanner fallback from `ks_p` to `logL_raw` in `file_ops.py` | same + `file_ops.py` | Click → partial file appears in results/ with `_partial_` tag |
...
| F-084 | **Live heatmap**: during run, shows Likelihood heatmap that progressively fills in as grid cells complete. Dsilva: uses `job['live_heatmaps']` via `polling.py`. **Langer: uses `_render_top_heatmaps_langer` via `polling_langer.py` with `job['_logL_raw_live']` — renders full H1-H4 grid live every 3s. WORKING ✓ (Langer 2026-03-30)** | `polling.py`, `polling_langer.py`, `cadence.py` | Heatmap partially filled during run; fully complete at end |
| F-085 | **Live 1D sigma profile**: line chart of max p-value vs σ_single. Only shown if sigma is scanned (n_sig > 1). Updates after each sigma slice completes. Uses `_make_max_pval_fig()` from helpers. **WORKING ✓ (2026-03-29)** | `polling.py`, `helpers.py`, `cadence.py` | Chart grows one point per sigma slice during run |
| F-086 | **Live 1D logPmax profile**: line chart of max p-value vs logP_max. Only shown if logPmax is scanned. Updates per logPmax slice. **WORKING ✓ (2026-03-29)** — added rendering in both live poll and final display in `polling.py` | `polling.py`, `helpers.py`, `cadence.py` | Same progressive update pattern as sigma profile |
...
| F-101 | **Summary table**: st.dataframe with columns — Best f_bin, 68% HDI f_bin, Best [π or σ or logP_max], 68% HDI, Best σ_single (if multi-sigma, else "X.XX (constant)"), Likelihood (best). Single row. **WORKING ✓** (Dsilva via `render_shared.py`, Langer via `render_shared_langer.py` — explicit 4-case grid handler, approved 2026-03-30) | `render_shared.py`, `render_shared_langer.py` (`_render_method_summary_section`) | Single row; HDI bounds make sense; best values match heatmap star; constant params annotated |
...
| F-114 | **Model Explorer**: interactive exploration of parameter space with live feedback. Sliders for f_bin [0,1], x_val [grid range], σ_single [grid range] (if multi-sigma), logPmax [grid range] (if present). Updates in real-time: CDF plot (observed + simulated median ± 68% band), ΔRV histogram, detection fraction metric, and per-method score value. Uses `_me_cdf_band()` (cached, N_sets=50). **WORKING ✓ (2026-03-29)**: 4 heatmaps with green dot, histogram overlay, detection fraction chart all approved. **MODIFY**: sliders reset, green dot on slider tracks, metric cards show −logL, CDF x-axis truncation | `render_lk_explorer.py` (`_render_lk_model_explorer`) | Move f_bin slider → CDF and histogram update; detection % changes |
| F-115 | **Detection fraction metric** (in Model Explorer): displays "Detected: X / Y (Z%)" where detection applies the full binary criterion: ΔRV > 45.5 km/s AND ΔRV − 4σ > 0. Shows how many simulated binaries would be classified as detected at the current parameter point. **WORKING ✓ (2026-03-29)** | `render_lk_explorer.py` | At f_bin=1.0 → high detection %; at f_bin=0.1 → fewer detections |
...
| F-131 | **Per-bin breakdown table**: st.dataframe with columns — Bin label (e.g., "0–45.5"), n_obs (observed count), n_sim (simulated count), p_i (sim fraction), ln(p_i), n_i·ln(p_i) (contribution to log-likelihood). Bottom row shows total ln L. **WORKING ✓** (Dsilva 2026-03-25, Langer 2026-03-30) | `render_lk_fit.py` (`_render_likelihood_stats_table`) | Sum of contributions = total ln L; zero-count bins handled |
...
| F-141 | **Gap analysis plot** (binary fraction vs ΔRV threshold): X = threshold (0 to 1.05×max_ΔRV), Y = fraction of stars above threshold. **Elements**: blue curve (simulated f_bin(T)), white step curve (real observed f_bin from `obs_delta_rv`, `shape='hv'`), red dashed horizontal (intrinsic f_bin target), amber dashed vertical (45.5 km/s threshold), **filled areas** (amber = missed binaries below threshold, blue = single-star false positives above), **diamond marker** at simulated point, **arrow annotation** showing gap between intrinsic and simulated f_bin, **gap statistics box**. **WORKING ✓** (Langer 2026-03-30 via `render_shared_langer.py`. Dsilva `render_shared.py` still has old "Observed" labels — needs same fix.) | `render_shared.py`, `render_shared_langer.py` | Both curves visible; step curve shows discrete jumps; gap annotation correct |
| F-142 | **Orbital histograms**: **9 parameter panels** (log₁₀P, e, q, K₁, M₁, M₂, i°, ω°, T₀ rad) in grid layout, 30 bins each. **5 view modes** via st.radio: "Compare detected vs missed" (red+amber), "Detected only", "Missed only", "All binaries (combined)", "Case A vs Case B" (Langer). Colors: red (detected), amber (missed), green (all). Constant parameters show vertical line instead of fake histogram. **WORKING ✓** (Langer 2026-03-30 via `render_shared_langer.py`. Dsilva `render_shared.py` still shows fake histogram for constant M₁ — needs same fix.) | `render_shared.py`, `render_shared_langer.py` | 9 panels visible; constant params show vertical line; radio changes coloring |
