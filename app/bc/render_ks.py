"""bc.render_ks — K-S entry point + heatmaps.

Renders the detailed analysis view when the user selects "K-S (standard)"
in the scoring method radio button.  Self-contained: copies all needed
helpers (no imports from analysis.py or scoring_detail.py).
"""
...
# Keys hardcoded for K-S
_METHOD_KEY = 'ks'
_DISPLAY_NAME = 'K-S (standard)'
_P_KEY = 'ks_p'
_D_KEY = 'ks_D'
_SCORE_LABEL = 'p-value'
...
# ─────────────────────────────────────────────────────────────────────────────
# Helper: max p-value vs scan variable line chart
# ─────────────────────────────────────────────────────────────────────────────
...
def render_ks_tab(p: str, ctx: dict, method_results: dict) -> None:
    """Render K-S (standard) detailed analysis tab.
...
# ─────────────────────────────────────────────────────────────────────────────
# K-S expander — primary heatmap + extra heatmaps + metrics + profiles
# ─────────────────────────────────────────────────────────────────────────────

def _render_ks_expander(
    p_nd: np.ndarray,
    ...
    # S_raw for K-S (unweighted)
    _ks_S_raw = _get_method_array(result, 'ks_S_raw')
    _ks_S_raw_2d = None
    if _ks_S_raw is not None:
        # Slice the same way as D/p arrays
        if _user_sig_idx is not None and _ks_S_raw.ndim >= 3:
            if ndim_mode == 'dsilva' and _ks_S_raw.ndim == 4:
                _lp_s2 = disp_outer_slices[0] if disp_outer_slices else 0
                _ks_S_raw_2d = _ks_S_raw[_lp_s2, _user_sig_idx]
            else:
                _ks_S_raw_2d = _ks_S_raw[_user_sig_idx]
        elif _ks_S_raw.ndim == 2:
            _ks_S_raw_2d = _ks_S_raw
        else:
            # Take best slice
            while _ks_S_raw.ndim > 2:
                _ks_S_raw = _ks_S_raw[0]
            _ks_S_raw_2d = _ks_S_raw

    try:
        from bc.render_ks_scoring import render_ks_scoring_detail
        render_ks_scoring_detail(
            ks_D_2d=D_2d if D_2d is not None else p_2d,
            ks_p_2d=p_2d,
            x_grid=fbin_g,
            y_grid=x_g,
            x_label='f_bin',
        ...
        st.warning('render_ks_scoring module not available.')
    ...
    # ── Corner plot ───────────────────────────────────────────────
    try:
        from bc.render_ks_fit import _render_ks_corner_plot
        _info = _render_ks_corner_plot(
    ...
    if _obs_drv_me is not None and _info is not None:
        ...
        with st.expander(f'Model Explorer -- {_DISPLAY_NAME}', expanded=False):
            try:
                from bc.render_ks_explorer import _render_ks_model_explorer
                _render_ks_model_explorer(
                    result, _METHOD_KEY, _DISPLAY_NAME, _SCORE_LABEL,
                    fbin_g, x_g, x_name, x_label, prefix, _info,
                    False, p_nd,
                )
            except ImportError:
                st.info('Model explorer not available.')
    ...
    if ndim_mode in ('cadence_dsilva', 'cadence_langer') and _info is not None:
        ...
        if _osc is not None:
            ...
            try:
                from bc.render_ks_explorer import _render_ks_cdf_sanity_check
