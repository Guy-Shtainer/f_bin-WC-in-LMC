"""bc.render_ks_scoring — K-S scoring detail heatmaps.

Renders the K-S-specific scoring analysis: log toggle, grid exclusion,
three scoring heatmaps (D5a-c), S_raw heatmap (D8), fit mode selector,
parabolic fitting, 3D surface, 1D slices, and interpolated best-fit.

Self-contained: all needed code is copied here (no imports from
analysis.py or scoring_detail.py).
"""
...
def render_ks_scoring_detail(
    ...
    """Render K-S scoring detail.  Returns (fit_coeffs, fit_info) for fitting step.
    ...
    # K-S labels (hardcoded, no likelihood branch)
    _stat_name = 'S'
    _stat_display = 'K-S D-statistic'
    _score_name = 'p-value'
    ...
    # K-S D-statistic: lower = better fit (no negation needed)
    _S_work = ks_D_2d.copy().astype(float)
    _S_work[_exc_mask_2d] = np.nan
    _p_work = ks_p_2d.copy().astype(float)
    _p_work[_exc_mask_2d] = np.nan

    # ── D5a: Raw D-statistic heatmap (all models) ────────────────
    st.markdown('#### K-S D-statistic Analysis')
    ...
    _raw_title = f'Weighted {_cbar_title} (all models)'
    ...
    # ── D5b: Score-masked D-statistic ─────────────────────────────
    S_masked = _S_work.copy()
    # K-S masking: p outside [0.05, 0.95] -> NaN
    p_mask = (_p_work < 0.05) | (_p_work > 0.95)
    S_masked[p_mask] = np.nan
    _masked_title = f'{_cbar_title} (p in [0.05, 0.95])'
    ...
    st.caption(
        'White = models outside p in [0.05, 0.95] (implausible).'
    )

    # ── D5c: Normalized p-value [0,1] ─────────────────────────────
    ...
    st.caption(
        'Fraction of simulated sets with S >= S_obs.'
    )

    # ── D8: S_raw heatmap (K-S only, unweighted) ─────────────────
    if (ks_S_raw_2d is not None
            and np.any(np.isfinite(ks_S_raw_2d))):
        _Sraw_work = ks_S_raw_2d
        fig_sraw = go.Figure(go.Heatmap(
            z=_to_display(_Sraw_work), x=y_grid, y=x_grid,
            colorscale='Viridis_r',
            colorbar=dict(
                title='log10(S_raw)' if _use_log else 'S_raw',
            ),
            hovertemplate=(
                f'{y_label}: %{{x:.3f}}<br>'
                f'{x_label}: %{{y:.3f}}<br>'
                f'S_raw: %{{z:.4f}}<extra></extra>'
            ),
        ))
        fig_sraw.update_layout(**{
            **_theme,
        ...
        st.caption(
            'Lower S_raw = better fit. '
            'Directly comparable between Dsilva and Langer models.'
        )
    ...
    fig_masked.add_trace(go.Scatter(
        ...
        hovertemplate=(
            ...
            f'S={best_S:.2f}<extra></extra>'
        ),
    ))
    _masked_slot.plotly_chart(fig_masked, use_container_width=(width is None))

    st.success(
        f'**Parabolic minimum (K-S):** {x_label} = {best_x:.4f}, '
    ...
    if _fit_coeffs is not None and _fit_bounds is not None:
        ...
        fig_3d.update_layout(**{
            **_theme,
            'title': dict(text='2D Parabolic Fit (K-S D-statistic)'),
            'scene': dict(
                xaxis_title=y_label,
                yaxis_title=x_label,
        ...
        if _do_3d_fit and _3d_coeffs is not None and _3d_bounds is not None:
            ...
            for _ip, (_gA, _gB, _lA, _lB, _lC, _bA, _bB, _bC,
                ...
                fig_proj.add_trace(go.Scatter3d(
                    ...
                    mode='markers',
                    marker=dict(
                        size=8, color='#DAA520', symbol='diamond'),
                    name='3D Minimum',
                    hovertemplate=(
